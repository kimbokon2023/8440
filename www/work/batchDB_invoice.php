    ...
    while ($row = $stmh->fetch(PDO::FETCH_ASSOC)) {
        ...
        switch ($worker) {
            ...
            case "최양원":
                ...
                break;
            case "임형주":
                $workertel = "010-8976-9777";
                break;
            case "박철우":
                $workertel = "010-4857-7022";
                break;
            case "조장우":
                $workertel = "010-5355-9709";
                break;
            case "백석묵":
                $workertel = "010-5635-0821";
                break;
            default:
                break;
        }	
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        <div class="d-flex justify-content-center align-items-center mb-2">
                                            ...
                                            <input type="date" id="fromdate" name="fromdate" class="form-control" style="width:100px;" value="<?php echo $fromdate; ?>">&nbsp; ~ &nbsp;
                                            <input type="date" id="todate" name="todate" class="form-control me-1" style="width:100px;" value="<?php echo $todate; ?>">
                                            
                                            <div class="inputWrap">
                                                <input type="text" id="search" name="search" style="width:150px;" value="<?php echo htmlspecialchars($search, ENT_QUOTES, 'UTF-8'); ?>" onkeydown="JavaScript:SearchEnter();" autocomplete="off" class="form-control me-1">
                                                <button class="btnClear"></button>
                                            </div>
                                            <button type="button" id="searchBtn" class="btn btn-dark btn-sm me-2"><i class="bi bi-search"></i> 검색</button>
                                        </div>
                                        <div class="d-flex justify-content-center align-items-center mb-2">
                                            <input type="date" id="recordDate" name="recordDate" class="form-control me-1" style="width:120px;" value="<?php echo $recordDate; ?>">
...
$(document).ready(function() {
    ...
    var num = <?php echo json_encode($num_arr ?: array(), JSON_UNESCAPED_UNICODE); ?> || [];
    var numcopy = new Array();
    var arr1 = <?php echo json_encode($endworkday_arr ?: array(), JSON_UNESCAPED_UNICODE); ?> || [];
    var arr2 = <?php echo json_encode($workday_arr ?: array(), JSON_UNESCAPED_UNICODE); ?> || [];
    var arr3 = <?php echo json_encode($work_order_arr ?: array(), JSON_UNESCAPED_UNICODE); ?> || [];
    var arr4 = <?php echo json_encode($workplacename_arr ?: array(), JSON_UNESCAPED_UNICODE); ?> || [];
    var arr5 = <?php echo json_encode($address_arr ?: array(), JSON_UNESCAPED_UNICODE); ?> || [];
    var arr6 = <?php echo json_encode($material_arr ?: array(), JSON_UNESCAPED_UNICODE); ?> || [];
    var arr7 = <?php echo json_encode($worker_arr ?: array(), JSON_UNESCAPED_UNICODE); ?> || [];
    var arr8 = <?php echo json_encode($item_arr ?: array(), JSON_UNESCAPED_UNICODE); ?> || [];
    var total_sum = 0;
    ...
    // 더블클릭 이벤트
    grid.on('dblclick', (e) => {
        var link = '<?php echo $base_url; ?>/work/view.php?menu=no&num=' + numcopy[e.rowKey];
    ...
    $("#invoiceBtn").click(function() {
        ...
        var col8Array = tmp.map(function(e) {
            return grid.getValue(e, 'col9');  // 각 행의 col9 값을 가져와 배열에 저장합니다.
        });
    ...
    $("#cancelBtn").click(function() {
        ...
        tmp.forEach(function(e) {
            grid.setValue(e, 'col2', '');  // 출고일에 넣어야 함
        });
        savegrid();
    });
    ...
    $("#saveDeadlineBtn").click(function() {
        var tmp = grid.getCheckedRowKeys();
        tmp.forEach(function(e) {
            grid.setValue(e, 'col1', $("#recordDate").val());  // 납기일 넣어야 함
        });
    ...
    $("#saveBtn").click(function() {
        var tmp = grid.getCheckedRowKeys();
        tmp.forEach(function(e) {
            grid.setValue(e, 'col2', $("#recordDate").val());  // 출고일에 넣어야 함
        });
        savegrid();
    });
