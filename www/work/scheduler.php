...
<head>
    ...
    <title>시공소장 작업스케줄러(SCHEDULER)</title>
...
<body>
    ...
    $radiopart = '';
    ?>
    <header class="d-flex flex-column align-items-center flex-md-row p-1 bg-secondary">
        <h1 class="h4 my-0 me-md-auto text-white">시공소장 작업일정(생산예정일 기준 다음날)</h1>
    ...
    <form name="Form">
        ...
        <input type="hidden" id="partOpt" name="partOpt" value="<?php echo htmlspecialchars($partOpt, ENT_QUOTES, 'UTF-8'); ?>">
    ...
    <span class="input-group-text bg-white">
        ...
        <button type="button" class="button btn btn-secondary" onclick="week_calandar(1)"> &gt; </button>
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 시공소장 스케줄 확인 및 시공전, 시공후 사진 등록 기록
        <?php echo $radiopart; ?>
    </span>

    <div id="calandarTitle"></div>
    <div id="calwrap" style="float:left;width:100%;height:900px;">
        <div id="calwrap_col1" style="float:left;width:50px;height:900px;margin-left:10px;">
        </div>
        <div id="calwrap_col2" style="float:left;width:1750px;height:900px;">
            <div id="calandar"></div>
        </div>
    </div>
...
function calendarMaker(target, date) {
    ...
    // 주간 창 만들어주기
    for (i = 0; i < 7 ; i++) {
        tag += "<td> <canvas id='myChart" + (i) + "' width='245' height='1500'></canvas> </td>";  // myChart 그려주기
        cnt++;
    }
    ...
    if (item == 'all') {
        ...
        for (var i = 0; i < arr.length; i++) {
            if (i % 3 == 0) {
                ...
                text1.set({ fill: '#000000' });
                canvas.add(text1);
            }
            if (i % 3 == 1) {
                var text1 = new fabric.Text(arr[i], { left: 50, top: 10 + i * 30, fontSize: 16 });
                text1.set({ fill: '#404040' });
                canvas.add(text1);
            }
            if (i % 3 == 2) {
                var text1 = new fabric.Text(arr[i], { left: 50, top: 10 + i * 30, fontSize: 16 });
                text1.set({ fill: '#00994C' });
                canvas.add(text1);
            }
        }
    }
