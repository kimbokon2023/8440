 ...
 $year=$_REQUEST["year"];   // 년도 체크박스
  ...
  $pdo = db_connect();
   ...
   while($row = $stmh->fetch(PDO::FETCH_ASSOC)) {
		      ...
		      $divide_take[$counter]=$row["holes"];   
		      $divide_take[$counter]=$row["take"];   
			  
			  if($divide_item[$counter]!=$last_item)
			  {
				 $last_item= $divide_item[$counter];
			     $divide_item[$item_counter]=$last_item;
				 $item_counter++;
			  }

			 /* print  $divide_num[$counter];			  
 			 print  $divide_item[$counter];
 			 print $divide_spec[$counter];
		     print $divide_take[$counter];    */
 ...
 <?php	 		  
	   ...
	   ?>		  
	   	   ...
	   	   <div id="which">   
			    ...
			    &nbsp;   <div id=which_b > 출고         <input  disabled type="radio" <?=$aryreg[1]?>  name=which value="2">  </div>  
	         </div>  		  
     <div class="clear"></div>			  
	 <div id="sero1"> 입출고일 :  </div>	 
	 <div id="sero2"> <input  disabled type="date" id="outdate" name="outdate" value="<?=$outdate?>" size="14" placeholder="출고일" > </div> 

	 <div id="sero3"> 접 수 일 :  </div>
	 <div id="sero2"> <input  disabled type="date" id="indate" name="indate" value="<?=$indate?>" size="14" placeholder="접수일" > </div> 
	      <div class="clear"></div>	
		  


	 <div id="sero3_1"> 현 장 명 :  </div>
	 <div id="sero5" style="width:700px;height:40px;"> <input  disabled type="text" style="height:30px;" id="outworkplace" name="outworkplace" onkeydown="JavaScript:Enter_Check();" value="<?=$outworkplace?>" size="60" placeholder="현장명"> 	 
	 ...
	 <div id="sero3" style="height:40px; width:80px; margin-top:15px; margin-left:35px;margin-right:-10px;"> 종 류 :  </div>
	    <div id="item1"> <select  disabled name="item" id="sel1"  >
           <?php		 
	 ...
	 <div id="sero44" > 홀 수 :  </div>		
	    <div id="item2"> <select  disabled name="holes" id="sel2" >
           <?php		 
	 ...
	 <div id="sero44" > 규격 :  </div>		
	    <div id="item3"> <select  disabled name="spec" id="sel3" >
           <?php		 
		   ...
		   for($i=0;$i<$spec_counter;$i++) {
				 ...
				 <input type="text"  disabled name="steelnum" id="steelnum" value="<?=$steelnum?>" size="8" placeholder="수량" onkeyup="inputNumberFormat(this)"  />
				 
			 
				 
	 <div class="clear"></div>
	 <div id="sero3" style="height:40px; margin-top:15px;"> 비 고 :  </div>
	 <div id="sero100" style="height:40px; margin-top:15px;"> <textarea  disabled rows="5" cols="80" name="comment" placeholder="기타사항 입력"
    ...
    <div class="clear"></div>	 
   	<!--   <div id="order5"> 수량 :  </div>		 --> 	 
		   ...
		   <div id="write_button_renew2">    
						...
						<a href="copy_data.php?mode=copy&num=<?=$num?>&page=<?=$page?>&search=<?=$search?>&find=<?=$find?>&process=<?=$process?>&yearcheckbox=<?=$yearcheckbox?>&year=<?=$year?>"><img src="../img/copydata.png"></a>&nbsp;
