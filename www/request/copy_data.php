...
<?php include "../common/modal.php"; ?>
  ...
  <form  id="board_form" name="board_form" method="post" onkeydown="return captureReturnKey(event)" action="insert.php?mode=copy&search=<?=$search?>&find=<?=$find?>&page=<?=$page?>&year=<?=$year?>&search=<?=$search?>&process=<?=$process?>&asprocess=<?=$asprocess?>&fromdate=<?=$fromdate?>&todate=<?=$todate?>&separate_date=<?=$separate_date?>"> 
	...
	<div class="d-flex mt-3 justify-content-start align-items-center"> 	    
       ...
       <?php 
		   ...
		   {
		?>			   
			<button id="backBtn" type="button" class="btn btn-dark  btn-sm me-2"  > <ion-icon name="arrow-back-outline"></ion-icon> 뒤로 이동  </button> 			
		<?php 
           }
		?>			   			
  ...
  <?php if($chkMobile===false)  
      ...
      <table class="table table-bordered">
        <tr>
          ...
          <td>
			<select name="steel_item" id="steel_item"  >
			   <?php
...
$("#rawmaterialBtn").click(function(){ 
 ...
 $("#saveBtn").click(function(){ 
    ...
    $('#requestdate').val($('#requestdate').val());
