...
<h2>납기일정</h2> <button id=insertBtn class="btn btn-outline-secondary "> 추가</button> 
</div>
	...
	$.ajax({
			...
			for(i = 0; i < Object.values(data)[0].length ; i++) {
				...
				if(Object.values(data)[0][i]['parent_id'] == '0')
				  ...
				  {
					titlename = Object.values(data)[0][i]['item_name'];	
					titlename += Object.values(data)[0][i]['send_company'];	
				  }
				...
				dataset.push({ title: titlename , start: new Date(y, m, d),  end: null , allDay: true , text: titlename , id :Object.values(data)[0][i]['id'] , parent_id : Object.values(data)[0][i]['parent_id'] });		
			}
