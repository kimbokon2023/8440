        ...
        <div class="card">
            ...
            <div class="card-body">
                <form id="board_form" name="board_form" method="post" enctype="multipart/form-data">
                    ...
                    <input type="hidden" id="rowkey" name="rowkey" value="<?=$rowkey?>">
                    ...
                    <div class="mb-4">
                        ...
                        <input
                            ...
                            id="spec"
                            name="spec"
                            value="<?=htmlspecialchars($spec, ENT_QUOTES, 'UTF-8')?>"
                            placeholder="<?=$title_message?>명을 입력하세요 (예: 1.2t 4'×8')"
                            required
                            autofocus
                        >
...
$(document).ready(function() {
    ...
    // 부모창으로 값 전달하는 함수
    function intoval(code, name) {
        // opener 변수변경 후 함수로 grid에 값을 넣어본다
        $("#prodcodechange", opener.document).val(code);
        $("#proddeschange", opener.document).val(name);
        $("#gridnum", opener.document).val('<?php echo $rowkey ?>');

        // opener.changeordergridcode();
        $(opener.location).attr("href", "javascript:changeordergridcode();");
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if (opener && !opener.closed) {
                    $("#search", opener.document).val($("#spec").val());
                    $(opener.location).attr("href", "javascript:reloadlist();");

                    // helloWorld 함수 호출 (있는 경우)
                    if (typeof opener.helloWorld === "function") {
                        opener.helloWorld();
                    }
                }
