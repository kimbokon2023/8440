...
if(isset($_REQUEST["param"])) 
{
  ...
  if(isset($_REQUEST["returnitem"]))   // 호출한 필드로 넣어주기 위해서 리턴변수 정한다.
	 $returnitem=$_REQUEST["returnitem"];	
	

if(isset($_REQUEST["search"]))   //목록표에 제목,이름 등 나오는 부분
	$search=$_REQUEST["search"];	  

if(isset($_REQUEST["rowkey"]))   //목록표에 제목,이름 등 나오는 부분
...
if(isset($_REQUEST["radio"]))   
	$radio = $_REQUEST["radio"];
	 ...
	 $radio = 'material';  // 라디오버튼 null인경우 material 설정   
 
isset($_REQUEST["text1"])  ? $text1 = $_REQUEST["text1"] :   $text1=""; 
isset($_REQUEST["text2"])  ? $text2 = $_REQUEST["text2"] :   $text2=""; 
isset($_REQUEST["text3"])  ? $text3 = $_REQUEST["text3"] :   $text3=""; 
...
$pdo = db_connect();	
  ...
  if(isset($_REQUEST["mode"]))
	...
	$a="  order by spec asc, num desc  ";  
	$b="  order by spec asc, num desc";
if(checkNull($search))
{
	$sql ="select * from mirae8440.steelspec where spec like '%$search%'  "  . $a;		
}
	else
		{
			$sql ="select * from mirae8440.steelspec  "  . $a;				
		}
    ...
    <tbody>					      	 
		<?php  
	       ...
	       while($row = $stmh->fetch(PDO::FETCH_ASSOC)) {
				include '_row.php';
			 ?>		
			<tr onclick="maketext('<?=$spec?>');return false;">
