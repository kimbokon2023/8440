...
$returnitem = $_REQUEST["returnitem"] ?? '';  // 호출한 필드로 넣어주기 위해서 리턴변수 정한다.
$search = $_REQUEST["search"] ?? '';  // 목록표에 제목,이름 등 나오는 부분
$rowkey = $_REQUEST["rowkey"] ?? '';
$radio = $_REQUEST["radio"] ?? 'material';  // 라디오버튼 null인경우 material 설정
...
if (checkNull($search)) {
    ...
    $searchParam = '%' . $search . '%';
    $sql = "select * from " . $DB . ".steelspec where spec like ? order by spec asc, num desc";
    $params = array($searchParam);
} else {
    $sql = "select * from " . $DB . ".steelspec order by spec asc, num desc";
    $params = array();
}
...
<form id="board_form" name="board_form" method="post" enctype="multipart/form-data">
    ...
    <input type="hidden" id="text1" name="text1" value="<?= htmlspecialchars($text1, ENT_QUOTES, 'UTF-8') ?>">
    <input type="hidden" id="text2" name="text2" value="<?= htmlspecialchars($text2, ENT_QUOTES, 'UTF-8') ?>">
    <input type="hidden" id="text3" name="text3" value="<?= htmlspecialchars($text3, ENT_QUOTES, 'UTF-8') ?>">
    <input type="hidden" id="param" name="param" value="<?= htmlspecialchars($param, ENT_QUOTES, 'UTF-8') ?>">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover" id="myTable">
                            ...
                            while ($row = $stmh->fetch(PDO::FETCH_ASSOC)) {
                                include '_row.php';
                            ?>
                                <tr onclick="maketext('<?= htmlspecialchars($spec, ENT_QUOTES, 'UTF-8') ?>');return false;">
                                    <td class="text-center"><?= $start_num ?></td>
                                    <td class="text-start"><?= htmlspecialchars($spec, ENT_QUOTES, 'UTF-8') ?></td>
...
var standardpageNumber; // 현재 페이지 번호 저장을 위한 전역 변수
...
$(document).ready(function() {
    ...
    // 페이지 번호 복원 (초기 로드 시)
    var savedPageNumber = getCookie('standardpageNumber');
    if (savedPageNumber) {
    ...
    dataTable.on('page.dt', function() {
        var standardpageNumber = dataTable.page.info().page + 1;
        setCookie('standardpageNumber', standardpageNumber, 10); // 쿠키에 페이지 번호 저장
    });
    ...
    $('#myTable_length select').on('change', function() {
        ...
        // 변경 후 현재 페이지 번호 복원
        savedPageNumber = getCookie('standardpageNumber');
        if (savedPageNumber) {
            dataTable.page(parseInt(savedPageNumber) - 1).draw(false);
        }
    });
...
function maketext(str) {
    ...
    if (window.opener && typeof window.opener.updateOptions === "function") {
        window.opener.updateOptions("spec", newValue);
    }
