...
print $_FILES[$filechoice]['name'][0];
...
if($filename !='') {   			
    ...
    $uploads_dir = '../imgceiling'; //업로드 폴더 상위 uploads 폴더선택
	...
	if($width>$height) {
     ...
     // insert
		try{		 
			...
			$sql = "insert into mirae8440.ceilpicfile ";
...
// log 기록 남기기
 ...
 $data=date("Y-m-d H:i:s") . " - " . $_SESSION["userid"] . " - " . $_SESSION["name"] . "  " . $workplacename . " - 포장사진" ;	
 require_once("../lib/mydb.php");
 $pdo = db_connect();
 $pdo->beginTransaction();
