...
if (!isset($_SESSION["level"]) || $level > 5) {
    ...
    $_SESSION["url"] = "{$base_url}/estimate/send_email_list/index.php";
    sleep(1);
...
// 전체 레코드 수 조회
// estimate_email_logs 테이블이 존재하는지 먼저 확인 (없으면 0)
try {
    $checkTable = $pdo->query("SHOW TABLES LIKE 'estimate_email_logs'");
...
<title>견적서 이메일 발송 내역 - 미래정공</title>
...
<div class="order-container">
    <div class="page-header">
        <h1><i class="fas fa-envelope-open-text"></i> 견적서 이메일 발송 내역</h1>
        <p>시스템에서 발송된 견적서 이메일 로그를 확인합니다.</p>
    </div>
                    ...
                    <?php foreach ($logs as $log): ?>
                        <tr>
                            ...
                            <td class="text-center">
                                <?php if ($log['estimate_id']): ?>
                                    <a href="../index.php?search_keyword=<?php echo $log['estimate_id']; ?>" target="_blank">
                    ...
                    <tr>
                        <td colspan="7">
                            <div class="empty-state">
                                ...
                                <p>견적서 관리에서 이메일을 발송하면 이곳에 기록됩니다.</p>
                            </div>
                        </td>
                    </tr>
                <?php endif; ?>
