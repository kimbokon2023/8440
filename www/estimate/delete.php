...
try {
    ...
    $check_sql = "SELECT id, supplier_name FROM `estimates` WHERE id IN ($placeholders) AND is_deleted = 0";
    ...
    if (empty($existing_orders)) {
        ...
        echo json_encode(['success' => false, 'message' => '존재하지 않는 견적서입니다.'], JSON_UNESCAPED_UNICODE);
        exit;
    }
    ...
    $delete_sql = "UPDATE `estimates` SET is_deleted = 1, updated_at = CURRENT_TIMESTAMP WHERE id IN ($delete_placeholders)";
    ...
    if ($delete_stmt->execute($existing_ids)) {
        ...
        if ($bulk) {
            echo json_encode([
                'success' => true,
                'message' => "{$deleted_count}개의 견적서가 삭제되었습니다.",
            ...
            // 로그 기록
            error_log("Bulk Estimate Delete: {$deleted_count} estimates deleted by {$user_name}, IDs: " . implode(',', $existing_ids));
        } else {
            // 단일 삭제의 경우 기존 응답 유지
            ...
            // 로그 기록
            error_log("Estimate deleted: ID={$order['id']}, Supplier={$order['supplier_name']}, User={$user_name}");
        }
    } else {
    ...
    // 데이터베이스 오류
    error_log("Estimate Delete Database Error: " . $e->getMessage());
    http_response_code(500);
