...
$pdo = db_connect();	
  ...
  if(isset($_REQUEST["mode"]))
	...
	$a="  order by company asc, num desc  ";  
	$b="  order by company asc, num desc";
if(checkNull($search))
{
	$sql ="select * from mirae8440.steelcompany where company like '%$search%'  "  . $a;	
	$sqlcon ="select * from mirae8440.steelcompany where company like '%$search%'    " . $b;	
}
	else
		{
			$sql ="select * from mirae8440.steelcompany  "  . $a;	
			$sqlcon ="select * from mirae8440.steelcompany  " . $b;	
		}
    ...
    <tbody>					      	 
		<?php  
	       ...
	       while($row = $stmh->fetch(PDO::FETCH_ASSOC)) {
				include '_row.php';
			 ?>		
			<tr onclick="maketext('<?=$company?>');return false;">
...
var outsourcingpageNumber; // 현재 페이지 번호 저장을 위한 전역 변수
...
$(document).ready(function() {			
    ...
    // 페이지 번호 복원 (초기 로드 시)
    var savedPageNumber = getCookie('outsourcingpageNumber');
    if (savedPageNumber) {
    ...
    dataTable.on('page.dt', function() {
        var outsourcingpageNumber = dataTable.page.info().page + 1;
        setCookie('outsourcingpageNumber', outsourcingpageNumber, 10); // 쿠키에 페이지 번호 저장
    });
    ...
    $('#myTable_length select').on('change', function() {
        ...
        // 변경 후 현재 페이지 번호 복원
        savedPageNumber = getCookie('outsourcingpageNumber');
        if (savedPageNumber) {
            dataTable.page(parseInt(savedPageNumber) - 1).draw(false);
        }
    });
...
{					                                           
	$("#company", opener.document).val(str); 	
	...
	if (window.opener && typeof window.opener.updateOptions === "function") {
		window.opener.updateOptions("company", newValue);
	}
	  self.close();		  
}	
