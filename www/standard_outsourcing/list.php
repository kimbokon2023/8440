...
if (checkNull($search)) {
    ...
    $searchParam = '%' . $search . '%';
    $sql = "select * from " . $DB . ".steelcompany where company like ? order by company asc, num desc";
    $params = array($searchParam);
} else {
    $sql = "select * from " . $DB . ".steelcompany order by company asc, num desc";
    $params = array();
}
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover" id="myTable">
                            ...
                            while ($row = $stmh->fetch(PDO::FETCH_ASSOC)) {
                                include '_row.php';
                            ?>
                                <tr onclick="maketext('<?= htmlspecialchars($company, ENT_QUOTES, 'UTF-8') ?>');return false;">
                                    <td class="text-center"><?= $start_num ?></td>
                                    <td class="text-start"><?= htmlspecialchars($company, ENT_QUOTES, 'UTF-8') ?></td>
