...
$comment10=$_REQUEST["comment10"];  
 ...
 $pdo = db_connect();
  ...
  if ($level=="7"){  // 서한컴퍼니인 경우는 update기록과 출하일자만 변경함  
...
// not level=7  
  ...
  if ($mode=="modify"){
    ...
    $sql .="type1=?, inseung1=?, su=?, bon_su=?, lc_su=?, etc_su=?, air_su=?, car_insize1=?, order_com1=?, order_text1=?,  order_com2=?, order_text2=?,  order_com3=?, order_text3=?,  order_com4=?, order_text4=?, ";  //16
	...
	$sql .=" third_item1=?, third_item2=?, third_item3=?, third_item4=?, forth_item1=?, forth_item2=?, forth_item3=?, forth_item4=?, fifth_item1=?, fifth_item2=?, fifth_item3=?, fifth_item4=?,  ";   //12
   ...
   try{
	...
	$sql .=" order_date1,  order_date2,  order_date3,  order_date4, order_input_date1, order_input_date2, order_input_date3, order_input_date4, ";   //8
	$sql .=" first_item1,  first_item2,  first_item3,  first_item4, second_item1, second_item2, second_item3, second_item4,  ";   //8
	$sql .=" third_item1,  third_item2,  third_item3,  third_item4, forth_item1, forth_item2, forth_item3, forth_item4, ";   //8
 ...
 } // end of $level =7 if statement
    ?>

 <?php
 
 if($mode=="not")
   header("Location:http://8440.co.kr/oem/list.php?num=$num&page=$page&search=$search&find=$find&list=1&process=$process&yearcheckbox=$yearcheckbox&year=$year&sortof=$sortof&cursort=$cursort&stable=1&check=$check&output_check=$output_check&plan_output_check=$plan_output_check&team_check=$team_check&measure_check=$measure_check&check_draw=$check_draw");    // 신규가입일때는 리스트로 이동
	 else		 
     header("Location:http://8440.co.kr/oem/view.php?num=$num&page=$page&search=$search&find=$find&list=1&process=$process&yearcheckbox=$yearcheckbox&year=$year&sortof=$sortof&cursort=$cursort&stable=1&check=$check&output_check=$output_check&plan_output_check=$plan_output_check&team_check=$team_check&measure_check=$measure_check&check_draw=$check_draw");   
