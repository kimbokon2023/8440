...
$forth_item1 = $_REQUEST["forth_item1"] ?? '';
$forth_item2 = $_REQUEST["forth_item2"] ?? '';
$forth_item3 = $_REQUEST["forth_item3"] ?? '';
$forth_item4 = $_REQUEST["forth_item4"] ?? '';
...
$car_insize1 = $_REQUEST["car_insize1"] ?? '';
$su = $_REQUEST["su"] ?? '';
...
// 카 사이즈 2~10 초기화
$car_insize2 = $_REQUEST["car_insize2"] ?? '';
$car_insize3 = $_REQUEST["car_insize3"] ?? '';
$car_insize4 = $_REQUEST["car_insize4"] ?? '';
$car_insize5 = $_REQUEST["car_insize5"] ?? '';
$car_insize6 = $_REQUEST["car_insize6"] ?? '';
$car_insize7 = $_REQUEST["car_insize7"] ?? '';
$car_insize8 = $_REQUEST["car_insize8"] ?? '';
$car_insize9 = $_REQUEST["car_insize9"] ?? '';
$car_insize10 = $_REQUEST["car_insize10"] ?? '';
...
if ($level == "7") {
    ...
    try {
        ...
        $sql = "UPDATE mirae8440.oem SET ";
        $sql .= "workday=?, delivery=?, delipay=?, update_log=? ";
        $sql .= "WHERE num=? LIMIT 1";
        
        $stmh = $pdo->prepare($sql);
        $stmh->bindValue(1, $workday, PDO::PARAM_STR);
        $stmh->bindValue(2, $delivery, PDO::PARAM_STR);
        $stmh->bindValue(3, $delipay, PDO::PARAM_STR);
        ...
        $pdo->rollBack();
        error_log("OEM 데이터 업데이트 오류 (서한컴퍼니, num: {$num}): " . $ex->getMessage());
        die("오류: 데이터 업데이트 중 문제가 발생했습니다.");
    }
} else {
    // Level 7이 아닌 경우
    if ($mode == "modify") {
        ...
        try {
            ...
            $sql .= "firstord=?, firstordman=?, firstordmantel=?, secondord=?, secondordman=?, secondordmantel=?, chargedman=?, chargedmantel=?, ";
            $sql .= "orderday=?, measureday=?, drawday=?, deadline=?, workday=?, worker=?, endworkday=?, material1=?, material2=?, material3=?, ";
            $sql .= "material4=?, material5=?, material6=?, widejamb=?, normaljamb=?, smalljamb=?, memo=?, ";
            $sql .= "regist_day=?, update_day=?, delivery=?, delicar=?, delicompany=?, delipay=?, delimethod=?, demand=?, startday=?, testday=?, hpi=?, first_writer=?, update_log=?, ";
            $sql .= "type1=?, inseung1=?, su=?, bon_su=?, lc_su=?, etc_su=?, air_su=?, car_insize1=?, order_com1=?, order_text1=?, order_com2=?, order_text2=?, order_com3=?, order_text3=?, order_com4=?, order_text4=?, ";
            $sql .= "lc_draw=?, lclaser_com=?, lclaser_date=?, lcbending_date=?, lcwelding_date=?, lcpainting_date=?, lcassembly_date=?, main_draw=?, eunsung_make_date=?, ";
            $sql .= "eunsung_laser_date=?, mainbending_date=?, mainwelding_date=?, mainpainting_date=?, mainassembly_date=?, memo2=?, ";
            $sql .= "order_date1=?, order_date2=?, order_date3=?, order_date4=?, order_input_date1=?, order_input_date2=?, order_input_date3=?, order_input_date4=?, ";
            $sql .= "first_item1=?, first_item2=?, first_item3=?, first_item4=?, second_item1=?, second_item2=?, second_item3=?, second_item4=?, ";
            $sql .= "third_item1=?, third_item2=?, third_item3=?, third_item4=?, forth_item1=?, forth_item2=?, forth_item3=?, forth_item4=?, fifth_item1=?, fifth_item2=?, fifth_item3=?, fifth_item4=?, ";
            $sql .= "sixth_item1=?, sixth_item2=?, sixth_item3=?, sixth_item4=?, seventh_item1=?, seventh_item2=?, seventh_item3=?, seventh_item4=?, ";
            $sql .= "eighth_item1=?, eighth_item2=?, eighth_item3=?, eighth_item4=?, ninth_item1=?, ninth_item2=?, ninth_item3=?, ninth_item4=?, ";
            $sql .= "tenth_item1=?, tenth_item2=?, tenth_item3=?, tenth_item4=?, ";
            ...
            $sql .= "car_insize2=?, car_insize3=?, car_insize4=?, car_insize5=?, car_insize6=?, car_insize7=?, car_insize8=?, car_insize9=?, car_insize10=?, ";
            ...
            $stmh->bindValue(49, $car_insize1, PDO::PARAM_STR);
            ...
            $stmh->bindValue(93, $forth_item1, PDO::PARAM_STR);
            $stmh->bindValue(94, $forth_item2, PDO::PARAM_STR);
            $stmh->bindValue(95, $forth_item3, PDO::PARAM_STR);
            $stmh->bindValue(96, $forth_item4, PDO::PARAM_STR);
            ...
            $stmh->bindValue(139, $car_insize2, PDO::PARAM_STR);
            $stmh->bindValue(140, $car_insize3, PDO::PARAM_STR);
            $stmh->bindValue(141, $car_insize4, PDO::PARAM_STR);
            $stmh->bindValue(142, $car_insize5, PDO::PARAM_STR);
            $stmh->bindValue(143, $car_insize6, PDO::PARAM_STR);
            $stmh->bindValue(144, $car_insize7, PDO::PARAM_STR);
            $stmh->bindValue(145, $car_insize8, PDO::PARAM_STR);
            $stmh->bindValue(146, $car_insize9, PDO::PARAM_STR);
            $stmh->bindValue(147, $car_insize10, PDO::PARAM_STR);
            ...
            $stmh->bindValue(158, $num, PDO::PARAM_STR);
            
            $stmh->execute();
            $pdo->commit();
        } catch (PDOException $ex) {
            $pdo->rollBack();
            error_log("OEM 데이터 수정 오류 (num: {$num}): " . $ex->getMessage());
            die("오류: 데이터 수정 중 문제가 발생했습니다.");
        }
    } else {
        // 신규 등록 모드
        ...
        try {
            ...
            $sql .= "orderday, measureday, drawday, deadline, workday, worker, endworkday, material1, material2, material3, ";
            $sql .= "material4, material5, material6, widejamb, normaljamb, smalljamb, memo, ";
            $sql .= "regist_day, update_day, delivery, delicar, delicompany, delipay, delimethod, demand, startday, testday, hpi, first_writer,";
            $sql .= "type1, inseung1, su, bon_su, lc_su, etc_su, air_su, car_insize1, order_com1, order_text1, order_com2, order_text2, order_com3, order_text3, order_com4, order_text4, ";
            $sql .= "lc_draw, lclaser_com, lclaser_date, lcbending_date, lcwelding_date, lcpainting_date, lcassembly_date, main_draw, eunsung_make_date, ";
            $sql .= "eunsung_laser_date, mainbending_date, mainwelding_date, mainpainting_date, mainassembly_date, memo2, ";
            $sql .= "order_date1, order_date2, order_date3, order_date4, order_input_date1, order_input_date2, order_input_date3, order_input_date4, ";
            $sql .= "first_item1, first_item2, first_item3, first_item4, second_item1, second_item2, second_item3, second_item4, ";
            $sql .= "third_item1, third_item2, third_item3, third_item4, forth_item1, forth_item2, forth_item3, forth_item4, ";
            $sql .= "fifth_item1, fifth_item2, fifth_item3, fifth_item4, sixth_item1, sixth_item2, sixth_item3, sixth_item4, ";
            $sql .= "seventh_item1, seventh_item2, seventh_item3, seventh_item4, eighth_item1, eighth_item2, eighth_item3, eighth_item4, ";
            $sql .= "ninth_item1, ninth_item2, ninth_item3, ninth_item4, tenth_item1, tenth_item2, tenth_item3, tenth_item4, ";
            $sql .= "type2, type3, type4, type5, type6, type7, type8, type9, type10, ";
            $sql .= "inseung2, inseung3, inseung4, inseung5, inseung6, inseung7, inseung8, inseung9, inseung10, ";
            $sql .= "car_insize2, car_insize3, car_insize4, car_insize5, car_insize6, car_insize7, car_insize8, car_insize9, car_insize10, ";
            $sql .= "comment1, comment2, comment3, comment4, comment5, comment6, comment7, comment8, comment9, comment10";
            ...
            $stmh->bindValue(48, $car_insize1, PDO::PARAM_STR);
            ...
            $stmh->bindValue(92, $forth_item1, PDO::PARAM_STR);
            $stmh->bindValue(93, $forth_item2, PDO::PARAM_STR);
            $stmh->bindValue(94, $forth_item3, PDO::PARAM_STR);
            $stmh->bindValue(95, $forth_item4, PDO::PARAM_STR);
            ...
            $stmh->bindValue(138, $car_insize2, PDO::PARAM_STR);
            $stmh->bindValue(139, $car_insize3, PDO::PARAM_STR);
            $stmh->bindValue(140, $car_insize4, PDO::PARAM_STR);
            $stmh->bindValue(141, $car_insize5, PDO::PARAM_STR);
            $stmh->bindValue(142, $car_insize6, PDO::PARAM_STR);
            $stmh->bindValue(143, $car_insize7, PDO::PARAM_STR);
            $stmh->bindValue(144, $car_insize8, PDO::PARAM_STR);
            $stmh->bindValue(145, $car_insize9, PDO::PARAM_STR);
            $stmh->bindValue(146, $car_insize10, PDO::PARAM_STR);
            ...
            $pdo->rollBack();
            error_log("OEM 데이터 등록 오류: " . $ex->getMessage());
            die("오류: 데이터 등록 중 문제가 발생했습니다.");
        }
    }
}
?>
...
if ($mode == "not") {
    // 신규 등록일 때는 리스트로 이동
    header("Location: {$base_url}/oem/list.php?{$params}");
} else {
    // 수정일 때는 상세보기로 이동
    header("Location: {$base_url}/oem/view.php?{$params}");
}
exit;
?>
