...
<head>
  ...
  <style>
    ...
    @media (max-width: 576px) {
      ...
      .mode-selector { font-size: 0.75rem; gap: 5px; flex-wrap: wrap; }
      .mode-selector .quiz-type { font-size: 0.75rem; }
      .mode-selector select { font-size: 0.75rem; }
      .col-sm-2 .runtime { font-size: 0.7rem; }
...
<body>
    ...
    <div class="row">
      <div class="col-sm-10">
        <div class="mode-selector" id="mode-selector">
		  ...
		  ( &nbsp; Web개발
          <div class="form-check form-check-inline">
            <input class="form-check-input" type="radio" name="quizType" id="html-quiz" value="html" checked>
            <label class="form-check-label" for="html-quiz">HTML</label>
          </div>
          <div class="form-check form-check-inline">
            <input class="form-check-input" type="radio" name="quizType" id="css-quiz" value="css">
            <label class="form-check-label" for="css-quiz">CSS</label>
          </div>
          <div class="form-check form-check-inline">
            <input class="form-check-input" type="radio" name="quizType" id="javascript-quiz" value="javascript">
            <label class="form-check-label" for="javascript-quiz">JavaScript</label>
          </div>
          <div class="form-check form-check-inline">
            <input class="form-check-input" type="radio" name="quizType" id="php-quiz" value="php">
            <label class="form-check-label" for="php-quiz">PHP</label>
          </div>
          <div class="form-check form-check-inline">
            <input class="form-check-input" type="radio" name="quizType" id="sql-quiz" value="sql">
            <label class="form-check-label" for="sql-quiz">SQL</label>
          </div>
		  )
          <div class="form-check form-check-inline ms-4">
            <input class="form-check-input" type="radio" name="quizType" id="python-quiz" value="python">
            <label class="form-check-label text-primary fw-bold" for="python-quiz">Python</label>
          </div>
          <span class="quiz-type ms-4 badge bg-secondary">HTML</span>
  ...
  <div class="shorts-header">
    <div class="channel-name">코딩 퀴즈왕</div>
  ...
  <script>
    ...
    function updateQuizTypeDisplay() {
      const quizTypeSpan = document.querySelector('.quiz-type');
      switch (quizType) {
        case 'html':
          quizTypeSpan.textContent = 'HTML 퀴즈';
          break;
        case 'css':
          quizTypeSpan.textContent = 'CSS 퀴즈';
          break;
        case 'javascript':
          quizTypeSpan.textContent = 'JavaScript 퀴즈';
          break;
        case 'python':
          quizTypeSpan.textContent = 'Python 퀴즈';
          break;
        case 'php':
          quizTypeSpan.textContent = 'PHP 퀴즈';
          break;
        case 'sql':
          quizTypeSpan.textContent = 'SQL 퀴즈';
          break;
        default:
          quizTypeSpan.textContent = 'HTML 퀴즈';
      }
      console.log(`Updated quiz type display to: ${quizTypeSpan.textContent}`);
    }

    async function loadQuizData(quizIndex = 0) {
      try {
        console.log(`Loading quiz data for type ${quizType}, index ${quizIndex}`);
        const jsonFile = quizType === 'html' ? './html/quizHtml.json' :
                         quizType === 'css' ? './html/quizCss.json' :
                         quizType === 'javascript' ? './html/quizJavascript.json' :
                         quizType === 'python' ? './html/quizPython.json' :
                         quizType === 'php' ? './html/quizPhp.json' :
                         quizType === 'sql' ? './html/quizSql.json' :
                         './html/quizHtml.json';
        ...
        if (quizIndex === 'random') {
          ...
          document.getElementById('shorts-title').textContent = `${quizType.charAt(0).toUpperCase() + quizType.slice(1)} 퀴즈`;
        } else {
          ...
          quizData = data[quizIndex].questions.slice(0, 5);
          document.getElementById('shorts-title').textContent = data[quizIndex].title || `${quizIndex + 1} 문제그룹`;
        }
    ...
    function showStartButton() {
      ...
      quizContainer.style.background = 'rgba(0, 0, 0, 0.7)';
    ...
    function setupEventListeners() {
      ...
      quizTypeRadios.forEach(radio => {
        radio.addEventListener('change', (e) => {
          console.log(`Quiz type changed to: ${e.target.value}`);
          quizType = e.target.value;
          updateQuizTypeDisplay();
          populateQuizSelect();
          loadQuizData('random').then(() => {
            console.log('Quiz data loaded for quiz type change, resetting quiz');
            resetQuiz();
          }).catch(err => {
            console.error('Failed to load quiz data on quiz type change:', err);
            quizData = FALLBACK_QUIZ.questions;
            document.getElementById('shorts-title').textContent = FALLBACK_QUIZ.title;
            resetQuiz();
          });
