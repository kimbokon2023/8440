...
<body>
    ...
    if ($mode == "modify" || $mode == "view") {
        try {
            ...
            error_log("연구노트 조회 오류: " . $ex->getMessage());
        }
    }
    ...
    if ($mode == "copy") {
        try {
            ...
            error_log("연구노트 복사 조회 오류: " . $ex->getMessage());
        }
                        ...
                        <div class="col-sm-5">
                            ...
                            if ($e_confirm === '' || $e_confirm === null) {
                                ...
                                if (empty($store)) {
                                    ...
                                    $parts = explode(' ', $store);
                                    $exam['date'] = array_pop($parts);
                                    $exam['date'] = array_pop($parts) . ' ' . $exam['date'];
                                    $exam['name'] = implode(' ', $parts);
                                }
                            ...
                            // 검토자 결재정보 생성
                            if (isset($exam['name']) && isset($exam['date'])) {
                                $store = $exam["name"] . ' ' . $exam['date'];
                            }

                            if ($status === 'end' && ($e_confirm !== '' && $e_confirm !== null)) {
                                ?>

                                <input type="hidden" id="store" name="store" value="<?=$store?>">
                                <input type="hidden" id="firstTime" name="firstTime" value="<?=isset($approvals[0]["date"]) ? $approvals[0]["date"] : ''?>">
                                <input type="hidden" id="secondTime" name="secondTime" value="<?=isset($approvals[1]["date"]) ? $approvals[1]["date"] : ''?>">
...
<div class="row d-flex justify-content-center mt-2">    
	 <textarea id="summernote" name="content" rows="18" required><?=$content?></textarea>      
</div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#summernote').on('summernote.change', function(we, contents, $editable) {
            ...
            // 1. 날짜 범위 추출 후 indate 설정
            var dateMatch = plainText.match(/\(\d{4}\.\d{2}\.\d{2} ~ (\d{4}\.\d{2}\.\d{2})\)/);
            if (dateMatch && dateMatch[1]) {
                var formattedDate = dateMatch[1].replace(/\./g, '-');
                $('#indate').val(formattedDate);
            }

            // 2. 주요 연구개발 내용 추출 후 mytitle 설정
            var titleMatch = plainText.match(/주요 연구개발 내용:\s*([^:\n]+)/);
            if (titleMatch && titleMatch[1]) {
                var title = titleMatch[1].trim();
                $('#mytitle').val('[본천장 자동작도] ' + title);
            }
        });
    });
</script>
