...
$options = ["설계", "레이져", "V컷", "절곡", "개발품"]; // '소장', '기타', '소재', '업체', '운반중' 제거

// 전체 출고 면적 및 전체 불량 면적 계산 (소장 제외)
$sql_total_area = "
    SELECT 
        SUM(CASE WHEN which = '2' THEN steelnum ELSE 0 END) AS total_area,
        SUM(CASE WHEN which = '2' AND bad_choice != '해당없음' AND bad_choice != '소장'  AND bad_choice != '업체'  AND bad_choice != '운반중'  AND bad_choice != '소재'  AND bad_choice != '기타' THEN steelnum ELSE 0 END) AS total_bad_area
