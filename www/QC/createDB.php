   ...
   if(makeweekdate== 1)  // 기존데이터가 '주간점검' 데이터를 없다면 생성한다
	  {
       for(i=0;i<mcno.length;i++)			  		
			writeDB('checkdate', dateFormat(Today), 'item', mcno[i] , 'term', '주간','writer', mcmain_arr[i],'writer2', mcsub_arr[i]);
	   }
   if(makemonthdate== 1)  // 기존데이터가 '1개월점검' 데이터를 없다면 생성한다
	  {
       for(i=0;i<mcno.length;i++)			  		
			writeDB('checkdate', dateFormat(Today), 'item', mcno[i] , 'term', '1개월','writer', mcmain_arr[i],'writer2', mcsub_arr[i]);
	   }
   if(maketwomonthdate== 1)  // 기존데이터가 '2개월점검' 데이터를 없다면 생성한다
	  {
       for(i=0;i<mcno.length;i++)			  		
			writeDB('checkdate', dateFormat(Today), 'item', mcno[i] , 'term', '2개월','writer', mcmain_arr[i],'writer2', mcsub_arr[i]);
	   }
   if(makesixmonthdate== 1)  // 기존데이터가 '6개월점검' 데이터를 없다면 생성한다
	  {
       for(i=0;i<mcno.length;i++)			  		
			writeDB('checkdate', dateFormat(Today), 'item', mcno[i] , 'term', '6개월','writer', mcmain_arr[i],'writer2', mcsub_arr[i]);
	   }	
	 }  // end of if
}
		
function writeDB(fn1,fv1,fn2,fv2,fn3,fv3,fn4,fv4,fn5,fv5) {        
       var fn = new Array();
       var fv = new Array();
