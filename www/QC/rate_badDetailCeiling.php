...
$sql_total_area = "
    SELECT 
        ...
        SUM(CASE WHEN which = '2' AND bad_choice != '해당없음' AND bad_choice != '소장'  AND bad_choice != '기타'  AND model ='천장'  THEN steelnum ELSE 0 END) AS total_bad_area
...
try {
    ...
    foreach ($options as $bad_choice) {
        $sql_bad_area = "
            SELECT 
                SUM(CASE WHEN which = '2' AND bad_choice = :bad_choice  AND model ='천장'  THEN steelnum ELSE 0 END) AS bad_area
            FROM mirae8440.steel
        ";
