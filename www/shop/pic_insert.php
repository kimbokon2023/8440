...
if($filename !='') {   			
   ...
   // $newfile=$tmpNm[0].".".$ext ;
	...
	//요기부분 수정했습니다.
	$filename1 = compress_image($_FILES['file']["tmp_name"][$i], $url1, 70); //실제 파일용량 줄이는 부분
	...
	if($width>$height) {
     ...
     // insert
		try{		 
			...
			$sql = "insert into mirae8440.picfile ";
...
// echo "<script> opener.document.getElementById('imgsrc').value='" . $url1 . "'; </script>";   // 부모창에 100 기록해보기
