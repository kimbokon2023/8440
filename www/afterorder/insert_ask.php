...
if ($mode == "modify") {
    try {
        $sql = "select * from mirae8440.afterorder where num=?";
    ...
    try {
        ...
        $pdo->rollBack();
        error_log("afterorder 수정 오류: " . $ex->getMessage());
    }
}
...
if ($mode == "insert") {
    try {
        ...
        $pdo->rollBack();
        error_log("afterorder 삽입 오류: " . $ex->getMessage());
    }
}
...
if ($mode == "delete") {
    try {
        ...
        $pdo->rollBack();
        error_log("afterorder 삭제 오류: " . $ex->getMessage());
    }
}
