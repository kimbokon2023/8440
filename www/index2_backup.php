...
<div class="offcanvas offcanvas-start" id="offcanvas">
	...
	<h2 class="offcanvas-title">캐릭터 보기</h2>
	<button type="button" class="btn-close text-reset" data-bs-dismiss="offcanvas"></button>
	</div>
	<div class="offcanvas-body">
	<?php   // include './work/load_statistics.php'; ?>
	<?php // include './ceiling/load_statistics.php'; ?>
			
		<?php
		$images = ['rabbit_small', 'turtle_small', 'horse_small', 'lamb_small', 'wolf_small', 'ostrich_small', 'leopard_small', 'goat_small', 'fox_small', 'puppy_small', 'lion_small', 'deer_small', 'cat_small', 'pig_small', 'giraffe_small', 'gorilla_small', 'donkey_small', 'person_small'];		

		// Loop through each image
		for ($i = 0; $i < count($images); $i++) {
			$imageSource = '../img/' . $images[$i] . '.gif';			
			echo '<span>';			
...
<div class="col-sm-3 board_list" style="padding:7;">
	<!-- Modern Light Blue Management Dashboard -->

	<div class="modern-management-card">
		<div class="modern-dashboard-header">
			📊 전일 경영 현황 <span style="font-size: 0.7rem; opacity: 0.9;">(단위: 원, SET)</span>
		</div>
...
<div class="col-sm-3 board_list" style="padding:7;">		
	...
	<table class="modern-dashboard-table">
		...
		<?php
				...
				$currentDate = new DateTime();				
					print "<thead> <tr>";
					print "<th class='text-center'> 순번 </th>";
					print "<th class='text-center'> 품명/현장명 </th>";
					print "<th class='text-center'> 받을분 </th>";
					print "</tr> </thead>";
				$start_num = 0;
				while($row = $stmh->fetch(PDO::FETCH_ASSOC)) {
					$start_num ++ ;
...
<div class="modern-management-card">
    ...
    <div class="modern-dashboard-body mt-1">
        <div class="row">
            ...
            <div class="col-sm-4">
              <div class="d-flex justify-content-start align-items-center">
                <button type="button" id="todo_view" class="btn btn-sm me-2 fw-bold" style="background: #0288d1; color: white; border: none; border-radius: 6px; padding: 0.25rem 0.5rem;">
                    <i class="bi bi-chevron-down"></i>
                </button>
                <h6 class="mb-0 me-3" style="color: #1e293b; font-weight: 600;">일정 관리</h6>
...
<?php     
if($user_name==='조경임' or $user_name==='소민지' )   
{
   ...
   if($total_row>0) 
	...
	// 조명/천장 월 달력출력	  
	// include "./ceiling/ceiling_schedule.php";  	
...
$('#text1').text('<?php echo $request_asked_count; ?>'); // 화면상단에 건수 표시  원자재 요청건
$('#text8').text('<?php echo $request_send_count; ?>'); // 화면상단에 건수 표시  원자재 발주보냄
$('#text2').text('<?php echo $request_etc_asked_count; ?>'); // 화면상단에 건수 표시 기타물품 요청건
$('#text7').text('<?php echo $request_etc_send_count; ?>'); // 화면상단에 건수 표시 기타물품 발주보냄
$('#text5').text('<?php echo $eat_count; ?>'); // 화면상단에 건수 표시 중식
$('#text6').text('<?php echo $aftereat_count; ?>'); // 화면상단에 건수 표시 석식
$('#lunch_text').text('<?php echo $lunch_text; ?>'); // 화면상단에 중식 종류
$('#dinner_text').text('<?php echo $dinner_text; ?>'); // 화면상단에 석식 종류
$('#lunch_done').text('<?php echo $lunch_done; ?>'); 
$('#supper_done').text('<?php echo $supper_done; ?>'); 
// 쟘 금일 정보 가져오기
$('#jamb_registedate').text('<?php echo $jamb_registedate; ?>'); 
$('#jamb_duedate').text('<?php echo $jamb_duedate; ?>'); 
$('#jamb_outputdonedate').text('<?php echo $jamb_outputdonedate; ?>'); 
// 천장 금일 정보 가져오기
$('#ceiling_registedate').text('<?php echo $ceiling_registedate; ?>'); 
$('#ceiling_duedate').text('<?php echo $ceiling_duedate; ?>'); 
$('#ceiling_outputdonedate').text('<?php echo $ceiling_outputdonedate; ?>'); 
// 덴크리 금일 정보 가져오기
$('#dancre_registedate').text('<?php echo $dancre_registedate; ?>'); 
$('#dancre_duedate').text('<?php echo $dancre_duedate; ?>'); 
$('#dancre_outputdonedate').text('<?php echo $dancre_outputdonedate; ?>'); 
// 다온텍 금일 정보 가져오기
