...
<script src="../js/html2canvas.js"></script>    <!-- 스크린샷을 위한 자바스크립트 함수 불러오기 -->
   ...
   <div id="wrap">
    ...
    if($mode=="modify"){
  ?>
	<form  name="board_form" method="post" onkeydown="return captureReturnKey(event)" action="insert.php?mode=modify&num=<?=$num?>&search=<?=$search?>&find=<?=$find?>&year=<?=$year?>&search=<?=$search?>&process=<?=$process?>&asprocess=<?=$asprocess?>&fromdate=<?=$fromdate?>&todate=<?=$todate?>&separate_date=<?=$separate_date?>" enctype="multipart/form-data"> 
  <?php  } else {
  ?>
	     ...
	     switch ($bend_state) {
			...
			default: break;
			 ...
			 ?>
			 	 ...
			 	 <div id="regist_state">   
	<b> 절곡접수 : &nbsp;   등록          <input  type="radio" <?=$aryreg[0]?> name=regist_state value="1">
 ...
 <div id="sero_new1">  &nbsp; &nbsp;   발주표시 :  &nbsp; &nbsp; </div>	
	...
	<div class="clear"></div>
	 ...
	 <div id="sero10"> <textarea rows="4" cols="140" name="comment" placeholder="기타 코멘트 남겨주세요." disabled
     ...
     <div class="clear"></div>  	 
            <div class="clear"></div>      <div class="clear"></div>      <br><br><br>	 
                  ...
                  <a href="#" onclick="window.open('print.php?num=<?=$num?>&upnum=<?=$upnum?>&page=<?=$page?>&search=<?=$search?>&find=<?=$find?>&list=1&process=<?=$process?>&sort=<?=$sort?>','발주내용 인쇄','left=20,top=20, scrollbars=yes, toolbars=no,width=1300,height=1450');" border="0" "> <img src="../img/printorder.png"></a>&nbsp;																		   
                        ...
                        print '
						&nbsp; <a href="process_DB.php?code=0&num=' . $num . '&search=' . $search . '&find=' . $find . '&fromdate=' . $fromdate . '&todate=' . $todate . '&separate_date=' . $separate_date . '"><img src="../img/input.png"></a> 
