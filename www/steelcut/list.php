...
$common="   where (" . $SettingDate . " between date('$fromdate') and date('$Transtodate')) and (steel='1') and (regist_state='2' or regist_state='3') order by " . $SettingDate;
  ...
  if($mode=="search"){
		  if($search==""){
             ...
             elseif($search!="") { // 각 필드별로 검색어가 있는지 쿼리주는 부분						
							  ...
							  $sql .=" (phone like '%$search%') or (comment like '%$search%')) and (steel='1') order by " . $SettingDate . " desc, num desc limit $first_num, $scale ";
							  ...
							  $sqlcon .=" (phone like '%$search%') or (comment like '%$search%')) and (steel='1') order by " . $SettingDate . "  desc, num desc ";							  
						}
               }
  if($mode=="") {
							 $sql="select * from chandj.output " . $a; 					
...
{	
                             $sql="select * from chandj.output " . $where . " order by indate desc  " . $all;           
          					 $sqlcon="select * from chandj.output " . $where . " order by indate desc, num desc  " ;            
}  
if($cursort==4) // 접수일 클릭시 정렬
{	
                             $sql="select * from chandj.output " . $where . " order by indate asc  " . $all;           
          					 $sqlcon="select * from chandj.output " . $where . " order by indate asc, num desc  " ;             
}             
...
<div id="col2">
  ...
  <div id="col2">     
	 <div id="title"><img src="../img/steelcut.png"> </div>	  
	       <div class="clear"></div>
 <!-- <div id="title2">
...
<!--      <input id ="thisyear" type='button' onclick='this_year()' value='당해년도'>		 						-->
      ...
      <div id="output_title14"> 절곡진행상태 </div>     
      ...
      <div id="list_content">
		  ...
		  if ($page<=1)  
	       ...
	       while($row = $stmh->fetch(PDO::FETCH_ASSOC)) {
			  ...
			  $bend_state=$row["bend_state"];	  
				if($bend_state==null)
			 $bend_state="1";
							switch ($bend_state) {
								case   "1"     :  $font_state="black"; $bend_word="등록"; break;
								case   "2"     :  $font_state="red"  ; $bend_word="접수"; break;	
								case   "3"     :  $font_state="blue"  ; $bend_word="완료"; break;	
								default:  $bend_word="등록"; break;
							}	
    ...
    $outdate = $outdate . $week[ date('w',  strtotime($outdate)  ) ] ;
			 ...
			 ?>
  			...
  			//	print '<div id class="blink" style="white-space:nowrap; color:green;" >';
				...
				<div id="outlist_item14" style="color:<?=$font_state?>;"> <?=substr($bend_word,0,8)?> </div>		
				<div id="outlist_item7"> 
				
				<?=iconv_substr($item_outworkplace,0,13,"utf-8")?> </div>
				<div id="outlist_item8"><?=iconv_substr($item_receiver,0,10,"utf-8")?></div>
		
				<div id="outlist_item9"><?=iconv_substr($item_outputplace,0,19,"utf-8")?></div>
				<div id="outlist_item10"><?=iconv_substr($item_phone,0,15,"utf-8")?></div>
				<div id="outlist_item11"><?=iconv_substr($item_orderman,0,5,"utf-8")?></div> 
				<div id="outlist_item12"><?=iconv_substr($item_comment,0,30,"utf-8")?></a></div>
		        <div class="clear"> </div>
				</div>
			<?php
			$start_num--;
 ...
 <?php
      if($page!=1 && $page>$page_scale)
      {
        ...
        print "<a href=list.php?page=$prev_page&mode=search&search=$search&find=$find&list=1&process=$process&asprocess=$asprocess&yearcheckbox=$yearcheckbox&year=$year>◀ </a>";
      }
    for($i=$start_page; $i<=$end_page && $i<= $total_page; $i++) 
      {        // [1][2][3] 페이지 번호 목록 출력
        if($page==$i) // 현재 위치한 페이지는 링크 출력을 하지 않도록 설정.
           ...
           print "<a href=list.php?page=$i&mode=search&search=$search&find=$find&list=1&process=$process&asprocess=$asprocess&yearcheckbox=$yearcheckbox&year=$year>[$i]</a>";
  }
      ...
      if($page<$total_page)
      {
        ...
        print "<a href=list.php?page=$next_page&mode=search&search=$search&find=$find&list=1&process=$process&asprocess=$asprocess&yearcheckbox=$yearcheckbox&year=$year> ▶</a><p>";
      }
 ?>			
        </div>
     </div>
