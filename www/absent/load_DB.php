...
$sql = "select * from mirae8440.member where part='제조파트' and eworks_level > 0 AND position !='퇴사' order by numorder asc";
try {
    $stmh = $pdo->query($sql);
    ...
    error_log("member 조회 오류: " . $ex->getMessage());
}
...
try {
    ...
    error_log("eworks 조회 오류: " . $ex->getMessage());
}
