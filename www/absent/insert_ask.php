...
if ($mode == "modify") {
    ...
    try {
        ...
        $sql = "update mirae8440.absent set id=?, name=?, registdate=?, item=?, askdatefrom=?, askdateto=?, usedday=?, content=?, state=?, part=?";
        ...
        $pdo->rollBack();
        error_log("absent 수정 오류: " . $ex->getMessage());
    }
}
...
if ($mode == "insert") {
    try {
        ...
        $sql = "insert into mirae8440.absent(id, name, registdate, item, askdatefrom, askdateto, usedday, content, state, part)";
        $sql .= " values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

        if ($name == '조경임' || $name == '김진억') {
            $state = '1차결재';
        } else {
            $state = '결재상신';
        }
        ...
        $pdo->rollBack();
        error_log("absent 삽입 오류: " . $ex->getMessage());
    }
}
...
if ($mode == "delete") {
    try {
        ...
        $sql = "delete from mirae8440.absent where num = ?";
        $stmh = $pdo->prepare($sql);
        $stmh->bindValue(1, $num, PDO::PARAM_STR);
        $stmh->execute();
        $pdo->commit();
        ...
        $pdo->rollBack();
        error_log("absent 삭제 오류: " . $ex->getMessage());
    }
}
