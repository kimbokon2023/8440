<?php
// SQL 쿼리를 작성하여 출고된 자재의 전체 생산 면적, 양산품 면적, 불량품 면적을 계산 (소장, 업체, 기타 제외)
$sql_total_area = "
    SELECT 
        SUM(CASE WHEN which = '2' THEN steelnum ELSE 0 END) AS total_area,
        SUM(CASE WHEN which = '2' AND bad_choice != '해당없음' AND bad_choice != '소장'  AND bad_choice != '업체'  AND bad_choice != '소재' AND bad_choice != '기타' THEN steelnum ELSE 0 END) AS defect_area
