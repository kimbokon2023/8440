...
// 임시로 기본 steelsource 테이블에서 이름가져와서 정렬 후 사용		 
...
// // 기존 자료 삭제
   // try{									
     ...
     // $sql = "delete from mirae8440.steelitem ";  
...
// // item 새로 써 넣기	
	...
	// for($i=0; $i<count($steelsource_item_yes); $i++) {	 
	   // try{
		 // $pdo->beginTransaction();		 
		 // $sql = "insert into mirae8440.steelitem (item) ";		 
		 // $sql .= "  values(?) ";
		 
		// $stmh = $pdo->prepare($sql); 
		// $stmh->bindValue(1, $steelsource_item_yes[$i] , PDO::PARAM_STR);               
 ...
 try{
	  $sql = "select * from mirae8440.steelitem order by item asc";
	  $stmh = $pdo->prepare($sql); 
	  $stmh->execute();
	  $count = $stmh->rowCount();              
	if($count<1){  
...
$(document).ready(function(){
			...
			$("#closeBtn").click(function(){    // 저장하고 창닫기	
					   ...
					   savegrid() ; // grid를 배열로 form에 전달한다.  						
						   ...
						   // data저장을 위한 ajax처리구문
							$.ajax({
								url: "registsteelitem_process.php",
								type: "post",		
					...
					var steelitem = <?php echo json_encode($steelitem);?> ;			
				...
				// grid 변경된 내용을 php 넘기기 위해 input hidden에 넣는다.
					...
					// 납품회사 숫자					
						...
						$("#item option", opener.document).remove();

					
							let steelitem   =  new Array(); 		
								
					        // console.log(grid.getRowCount());	//삭제시 숫자가 정상적으로 줄어든다.
						     const MAXcount=grid.getRowCount() ;  // + 20 적용여부 확인 20개 데이터를 rowkey 영향으로 더 검색한다.						     
							 let pushcount=0;
							 let tmp='';
							 
							 for(i=0;i<MAXcount;i++) {      // grid.value는 중간중간 데이터가 빠진다. rowkey가 삭제/ 추가된 것을 반영못함.    
							    tmp = grid.getValue(i, 'steelitem');
							    if( tmp != null ) {									
								    steelitem.push(tmp);
									// 부모창에 적용해 주기 opener 활용기법
									$("#item", opener.document).append("<option value='" + tmp + "'>" +tmp + "</option>");

									} // end of else
								   									
								 }	
								 
								 // console.log(steelitem);
								 
								$('#steelitem').val(steelitem);				
