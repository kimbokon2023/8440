...
$search = getRequestValue("search", '');  // 검색어
$find = getRequestValue("find", '');  // 검색항목
$page = getRequestValue("page", 1);  // 페이지번호
$process = getRequestValue("process", '');  // 진행현황
$year = getRequestValue("year", '');  // 년도
$yearcheckbox = getRequestValue("yearcheckbox", '');  // 년도 체크박스

// 체크박스 변수들 초기화
$check = getRequestValue("check", '0');  // 미출고 리스트
$output_check = getRequestValue("output_check", '0');  // 출고 체크
$team_check = getRequestValue("team_check", '0');  // 시공팀미지정
$measure_check = getRequestValue("measure_check", '0');  // 미실측리스트
$plan_output_check = getRequestValue("plan_output_check", '0');  // 출고예정

// 정렬 관련 변수
$cursort = getRequestValue("cursort", '0');  // 정렬
$sortof = getRequestValue("sortof", '0');  // 정렬기준
$stable = getRequestValue("stable", '0');  // 테이블
...
try {
    ...
    if ($row) {
        ...
        $imgurl1 = $filename1 ? "../imgwork/" . $filename1 : '';
        $imgurl2 = $filename2 ? "../imgwork/" . $filename2 : '';
    }
} catch (PDOException $Exception) {
    error_log("데이터베이스 오류: " . $Exception->getMessage());
    print "오류: 데이터를 불러오는 중 문제가 발생했습니다.";
}
    ...
    <div class="container-fluid">
        ...
        <div class="row mb-4">
            <div class="col-12 d-flex flex-wrap gap-2 align-items-center">
                <button type="button" class="btn btn-secondary btn-lg" onclick="move_url('index.php');">
                    목록으로 돌아가기
                </button>
                <button type="button" class="btn btn-primary btn-lg" onclick="input_measureday('process_DB.php?num=<?=htmlspecialchars($num, ENT_QUOTES, 'UTF-8')?>')">
                    실측완료 전송하기
                </button>
                <button type="button" class="btn btn-warning btn-lg" onclick="input_message('reg_pic.php?num=<?=htmlspecialchars($num, ENT_QUOTES, 'UTF-8')?>')">
                    시공전후 사진등록
                </button>
            </div>
        </div>
            ...
            <div class="container">
                ...
                <div class="row">
                    <h1 class="display-5 font-center text-left">
                        ...
                        막판 HPI 형태 : <span style="color:brown;font-weight:bold;"><?=htmlspecialchars($hpi, ENT_QUOTES, 'UTF-8')?></span>
                        <br><br>

                        재질 1 : <input disabled type="text" value="<?=htmlspecialchars($material1 . ' ' . $material2, ENT_QUOTES, 'UTF-8')?>" size="25"><br>
                        ...
                        <span style="color:blue;font-weight:bold;font-size:50px;">현장소장 : <?=htmlspecialchars($worker, ENT_QUOTES, 'UTF-8')?></span>
                        <br><br>

                        현장주소 : <?=htmlspecialchars($address, ENT_QUOTES, 'UTF-8')?>
                        <br><br>
                        
                        원  청 : <?=htmlspecialchars($firstord, ENT_QUOTES, 'UTF-8')?>
                        <br>
                        원청담당(PM) : <?=htmlspecialchars($firstordman, ENT_QUOTES, 'UTF-8')?>
                        <br>
                        연락처 : <a href="tel:<?=htmlspecialchars($firstordmantel, ENT_QUOTES, 'UTF-8')?>"><?=htmlspecialchars($firstordmantel, ENT_QUOTES, 'UTF-8')?></a>
                        <br><br>
                        ...
                        <br>
                        연락처 : <a href="tel:<?=htmlspecialchars($secondordmantel, ENT_QUOTES, 'UTF-8')?>"><?=htmlspecialchars($secondordmantel, ENT_QUOTES, 'UTF-8')?></a>
                        <br><br>
                        
                        현장소장 : <?=htmlspecialchars($chargedman, ENT_QUOTES, 'UTF-8')?>
                        <br>
                        현장소장연락처 : <a href="tel:<?=htmlspecialchars($chargedmantel, ENT_QUOTES, 'UTF-8')?>"><?=htmlspecialchars($chargedmantel, ENT_QUOTES, 'UTF-8')?></a>
                        <br><br>

                        발주접수일: <input disabled type="text" name="orderday" id="orderday" value="<?=htmlspecialchars($orderday, ENT_QUOTES, 'UTF-8')?>" size="10" placeholder="발주접수일">
                        <br>
                        실측일 : <input disabled type="text" value="<?=htmlspecialchars($measureday, ENT_QUOTES, 'UTF-8')?>" size="10" placeholder="실측일">
                        <br>
                        도면설계완료일 : <input disabled type="text" value="<?=htmlspecialchars($drawday, ENT_QUOTES, 'UTF-8')?>" size="10" placeholder="도면설계일">
                        <br><br>

                        제품출고일: <?=htmlspecialchars($workday, ENT_QUOTES, 'UTF-8')?>
                        <br><br>
                        
                        착공일: <?=htmlspecialchars($startday, ENT_QUOTES, 'UTF-8')?><br>
                        검사일 : <input disabled type="text" value="<?=htmlspecialchars($testday, ENT_QUOTES, 'UTF-8')?>" size="10" style="color:red;">
                        <br><br><br>
                        
                        추가 메모(기타 사항) :
                        <div class="clear"></div>
                        <div class="row"></div>
                        <textarea disabled rows="3" cols="30" name="memo" placeholder="추가적으로 기록할 내역"><?=htmlspecialchars($memo, ENT_QUOTES, 'UTF-8')?></textarea>
                        <br><br>
                        ...
                        try {
                            ...
                            error_log("VOC 조회 오류: " . $Exception->getMessage());
                            print "오류: 협의사항을 불러오는 중 문제가 발생했습니다.";
                        }
                        ?>

                        협의사항 기록 : <br>
                        <textarea disabled rows="10" cols="30" name="content" placeholder="협의사항 기록 내역" style="color:brown;"><?=htmlspecialchars($content, ENT_QUOTES, 'UTF-8')?></textarea>
                        <br><br>
                        
                        청구일자 : <input disabled type="text" name="demand" id="demand" value="<?=htmlspecialchars($demand, ENT_QUOTES, 'UTF-8')?>" size="15" placeholder="청구일, 계산서발행">

                        <div class="clear"></div>
                        <br><br>
                        
                        <div class="row">
                            <h1 class="display-4 font-center text-center" style="color:blue;margin-top:30px;">시공 전 사진</h1>
                        </div>
                        <div class="clear"></div>
                        <div class='imagediv'>
                            <?php if (!empty($filename1) && !empty($imgurl1)): ?>
                                <img class="before_work" src="<?=htmlspecialchars($imgurl1, ENT_QUOTES, 'UTF-8')?>" alt="시공 전 사진">
                            <?php endif; ?>
                        </div>
                        <div class="clear"></div>
                        <br><br>
                        ...
                        <div class="row">
                            <h1 class="display-4 font-center text-center" style="color:red;margin-top:30px;">시공 후 사진</h1>
                        </div>
                        <div class="clear"></div>
                        <div class='imagediv'>
                            <?php if (!empty($filename2) && !empty($imgurl2)): ?>
                                <img class="after_work" src="<?=htmlspecialchars($imgurl2, ENT_QUOTES, 'UTF-8')?>" alt="시공 후 사진">
                            <?php endif; ?>
                        </div>
                        <div class="clear"></div>
                    </h1>
...
(function() {
    ...
    function rotate_image() {
        ...
        $('.before_work, .after_work').addClass('rotated');
    }
    
    // 페이지 로드 후 1.5초 뒤에 이미지 회전
    setTimeout(function() {
        rotate_image();
    }, 1500);
