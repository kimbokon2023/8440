...
<br> 
			...
			<h1 class="display-2  text-left">
			  <input type="button" class="btn btn-secondary btn-lg " value="목록으로 돌아가기" onclick="javascript:move_url('index.php');"> </h1>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<h1 class="display-2  text-left">
			  <input type="button" class="btn btn-primary btn-lg " value="실측완료 전송하기" onclick="javascript:input_measureday('process_DB.php?num=<?=$num?>')"> </h1>
			  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			  <h1 class="display-2  text-left">
			  <input type="button" class="btn btn-warning btn-lg " value="시공전후 사진등록" onclick="javascript:input_message('reg_pic.php?num=<?=$num?>')"> </h1>    
			  </div>
<br>
<br>
<form id="board_form" name="board_form" method="post" action="view.php?mode=search&search=<?=$search?>&find=<?=$find?>&process=<?=$process?>&yearcheckbox=<?=$yearcheckbox?>&year=<?=$year?>&check=<?=$check?>&output_check=<?=$output_check?>&plan_output_check=<?=$plan_output_check?>&team_check=<?=$team_check?>&measure_check=<?=$measure_check?>&page=<?=$page?>&cursort=<?=$cursort?>&sortof=<?=$sortof?>&stable=<?=$stable?>&scale=10000">  
     <div  class="container">
			<div class="row">
     ...
     <br>
     <br>
	 	<span style="color:blue;font-weight:bold;"> 출고예정일(김진억이사 협의) : </span> <span style="color:red;font-weight:bold;">  <?=$endworkday?>  </span>
<br> 	
<br> 	
	 ...
	 <?php
	  ...
	  if($smalljamb>0) 	   	   
	   ...
	   ?>
			<br> 
			<br> 
		<span style="color:blue;font-weight:bold;font-size:50px;"> 현장소장 : <?=$worker?>  </span>	
			<br> 
			<br> 
...
<br>
  ...
  <textarea disabled rows="3" cols="30" name="memo" placeholder="추가적으로 기록할 내역" ><?=$memo?></textarea>
       <br>
...
<?php
 ...
 ?>
  ...
  협의사항 기록 : <br> <textarea disabled rows="10" cols="30" name="memo" placeholder="협의사항 기록 내역" style="color:brown;" ><?=$content?></textarea>
       <br>		   	   
       <br>
	   청구일자 : <input  disabled type="text" name="demand" id="demand" value="<?=$demand?>" size="15" placeholder="청구일, 계산서발행"  >  
