 ...
 if ($mode=="modify"){
     ...
     try{
        $sql = "select * from chandj.free where num=?";  // get target record
      ...
      for ($i=0; $i<$count; $i++)						
      {
		...
		unlink($delete_path);
		   ...
		   $pdo->beginTransaction(); 
                   $sql = "update chandj.free set $field_org_name = ?, $field_real_name = ?  where num=?";
                   $stmh = $pdo->prepare($sql); 
     ...
     try{
        $pdo->beginTransaction();   
        $sql = "update chandj.free set subject=?, content=?, is_html=? where num=?";
        $stmh = $pdo->prepare($sql); 
   ...
   try{
     $pdo->beginTransaction();
     $sql = "insert into chandj.free(id, name, nick, subject, content, regist_day, hit, is_html, ";
   ...
   header("Location:http://5130.co.kr/free/list.php");
 ?>
