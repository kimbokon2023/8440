...
$all=" limit $first_num, $scale";
  ...
  if($mode=="search"){
		  if($search==""){
							 $sql="select * from mirae8440.output " . $a; 					
	                         $sqlcon = "select * from mirae8440.output " . $b;   // 전체 레코드수를 파악하기 위함.					
			       }
             elseif($search!="") { // 각 필드별로 검색어가 있는지 쿼리주는 부분						
							  $sql ="select * from mirae8440.output where (outdate like '%$search%')  or (outworkplace like '%$search%') ";
							  ...
							  $sqlcon ="select * from mirae8440.output where (outdate like '%$search%')  or (outworkplace like '%$search%') ";
...
if($cursort==1)
{	
          					 $sql="select * from mirae8440.output  " . $c;          
          					 $sqlcon="select * from mirae8440.output  " . $d;          
}  
 
if($cursort==2)
{	
                             $sql="select * from mirae8440.output   " . $a;           
          					 $sqlcon="select * from mirae8440.output " . $b;         
}  
if($cursort==3) // 접수일 클릭시 정렬
{	
                             $sql="select * from mirae8440.output " . $where . " order by indate desc  " . $all;           
          					 $sqlcon="select * from mirae8440.output " . $where . " order by indate desc  " ;            
}  
if($cursort==4) // 접수일 클릭시 정렬
{	
                             $sql="select * from mirae8440.output " . $where . " order by indate asc  " . $all;           
          					 $sqlcon="select * from mirae8440.output " . $where . " order by indate asc  " ;             
}  
if($cursort==5) // 구분 클릭시 주일/경동 내림 정렬
{	
                             $sql="select * from mirae8440.output " . $where . " order by root desc, outdate desc  " . $all;           
          					 $sqlcon="select * from mirae8440.output " . $where . " order by root desc, outdate desc  " ;         
}     

if($cursort==6) // 구분 클릭시 주일/경동 오름차순 정렬
{	
                             $sql="select * from mirae8440.output" . $where . " order by root asc, outdate desc  " . $all;           
          					 $sqlcon="select * from mirae8440.output" . $where . " order by root asc, outdate desc  " ;       
}        
if($cursort==7) // 절곡 클릭시 내림 정렬
{	
                             $sql="select * from mirae8440.output" . $where . " order by steel desc, outdate desc  " . $all;           
          					 $sqlcon="select * from mirae8440.output" . $where . " order by steel desc, outdate desc  ";         
}        
if($cursort==8) // 절곡 클릭시 오름차순 정렬
{	
                             $sql="select * from mirae8440.output" . $where . " order by steel asc, outdate desc  " . $all;           
          					 $sqlcon="select * from mirae8440.output" . $where . " order by steel asc, outdate desc  ";     
}           
if($cursort==9) // 모터 클릭시 내림 정렬
{	
                             $sql="select * from mirae8440.output" . $where . " order by motor desc, outdate desc  " . $all;           
          					 $sqlcon="select * from mirae8440.output" . $where . " order by motor desc, outdate desc  " ;          
}        
if($cursort==10) // 모터 클릭시 오름차순 정렬
{	
                             $sql="select * from mirae8440.output" . $where . " order by motor asc, outdate desc  " . $all;           
          					 $sqlcon="select * from mirae8440.output" . $where . " order by motor asc, outdate desc  " ;       
}              
