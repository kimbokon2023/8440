...
// 복사 모드일 경우를 위한 이전 번호 저장
$oldnum = $num;	

// 검색 태그 초기화 및 생성
$searchtag = '';

// 조건에 따라 검색 태그에 문자열 추가
if (!empty($part1)) {
    $searchtag .= '중국산휀, ';
}
if (!empty($part2)) {
...
// 수정 모드 (modify)
if ($mode == "modify") {
    // 업데이트 로그에 현재 수정자 정보 추가
    $data = date("Y-m-d H:i:s") . " - " . $session_name . "  ";
    $update_log = $data . $update_log . "&#10";  // 개행문자 Textarea용
...
// INSERT 모드 (신규 등록 또는 복사)
else {
    // 최초 등록자 기록
    $first_writer = $session_name . " _" . date("Y-m-d H:i:s");
    
    // 복사가 아닌 경우 update_log 초기화
    if ($mode != 'copy') {
        $update_log = '';
    }
