...
// 변수 초기화
$chargedperson = isset($_REQUEST["chargedperson"]) ? $_REQUEST["chargedperson"] : "";
$childnum = isset($_REQUEST["childnum"]) ? $_REQUEST["childnum"] : "";
...
try {
    ...
    if ($row) {
        ...
        $PROD_DES = isset($row["PROD_DES"]) ? $row["PROD_DES"] : "";
        $filename1 = isset($row["filename1"]) ? $row["filename1"] : "";
        $imgurl1 = isset($row["imgurl1"]) ? $row["imgurl1"] : "";
    }
