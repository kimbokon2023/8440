...
<?php
    ...
    error_log("DB query error in idea/search_ceiling.php: " . $ex->getMessage());
    ?>
...
<div id="page_button">
    <div id="page_num">
        ...
        if ($page != 1 && $page > $page_scale) {
            ...
            $prevUrl = 'search_ceiling.php?' . http_build_query(array(
                'page' => $prev_page,
                'search' => $search,
                'find' => $find,
                'list' => 1,
        ...
        for ($i = $start_page; $i <= $end_page && $i <= $total_page; $i++) {
            if ($page == $i) {
                ...
                $pageUrl = 'search_ceiling.php?' . http_build_query(array(
                    'page' => $i,
                    'search' => $search,
                    'find' => $find,
                    'list' => 1,
        ...
        if ($page < $total_page) {
            ...
            $nextUrl = 'search_ceiling.php?' . http_build_query(array(
                'page' => $next_page,
                'search' => $search,
                'find' => $find,
                'list' => 1,
