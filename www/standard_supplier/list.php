...
if (checkNull($search)) {
    ...
    $searchParam = '%' . $search . '%';
    $sql = "select * from " . $DB . ".steelsupplier where company like ? order by company asc, num desc";
    $params = array($searchParam);
} else {
    $sql = "select * from " . $DB . ".steelsupplier order by company asc, num desc";
    $params = array();
}
...
var supplierpageNumber; // 현재 페이지 번호 저장을 위한 전역 변수
...
$(document).ready(function() {
    ...
    // 페이지 번호 복원 (초기 로드 시)
    var savedPageNumber = getCookie('supplierpageNumber');
    if (savedPageNumber) {
    ...
    dataTable.on('page.dt', function() {
        var supplierpageNumber = dataTable.page.info().page + 1;
        setCookie('supplierpageNumber', supplierpageNumber, 10); // 쿠키에 페이지 번호 저장
    });
    ...
    $('#myTable_length select').on('change', function() {
        ...
        // 변경 후 현재 페이지 번호 복원
        savedPageNumber = getCookie('supplierpageNumber');
        if (savedPageNumber) {
            dataTable.page(parseInt(savedPageNumber) - 1).draw(false);
        }
    });
...
function maketext(str) {
    // 공급처는 supplier
    $("#supplier", opener.document).val(str);
    ...
    if (window.opener && typeof window.opener.updateOptions === "function") {
        window.opener.updateOptions("supplier", newValue);
    }
    self.close();
}
