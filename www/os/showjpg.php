...
$pdo = db_connect();
     ...
     try{
      $sql = "select * from mirae8440.os where num = ? ";
...
<div id="outlineprint">  
	...
	<div class="clear"> </div>
    <div id="row2">   <?=$secondord?> </div>
    <div class="clear"> </div>			
    <div id="row3">  <?=$et_wpname?>  </div>   
    <div class="clear"> </div>		
    <div id="row4">   <?=$workplacename?> </div>        
	<div class="clear"> </div>	
    <div id="row5">   <?=$et_schedule?> </div>           
	<div class="clear"> </div>	
    <div id="row6">   <?=$et_person?> </div>           
	<div class="clear"> </div>	
    <div id="row6-1">  </div>           
	<div class="clear"> </div>		
        
	<?php
			for($i=0 ; $i < 20 ; $i++) 	 // 5개의 데이터 기준으로 찍고 하단에 totalamount 기록하기 위함
			{ 
			  if($description[$i]!=null) 
			  {				  
				   print '<div id="row7"> ';
						print '<div id="col1"> '.  (int)($i+1)  . '</div>';			
						print '<div id="col2"> '.  $description[$i]  . '</div>';			
						print '<div id="col3"> '.  $spec[$i]  . '</div>';			
						print '<div id="col4"> '.  $ea[$i]  . '</div>';	
					   if((int)$unit[$i] > 0)				
							print '<div id="col5"> '.  number_format($unit[$i])  . '</div>';			
						  else
							print '<div id="col5"> </div>';			
						if((int)$amount[$i] > 0)				
							print '<div id="col6"> '.  number_format($amount[$i])  . '</div>';			
						  else
	  ...
	  ?>
		<div class="clear"> </div>		  
		<div id="row8"> <?=$totalamount?> </div>           
		<div class="clear"> </div>		  
		<div id="row9"> <?=$et_note?> </div>           
		<div class="clear"> </div>	
...
//jpg 결과값
  ...
  $.ajax({
    type: 'post',
    url: 'http://8440.co.kr/os/imgupload.php',
    cache: false,
    data: formData,
    processData: false,
    contentType: false,
