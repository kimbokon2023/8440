...
$b= $attached .  " " . $orderby;
		  ...
		  if($search==""){
					 $sql="select * from mirae8440.os  " . $a; 					
	                 $sqlcon = "select * from mirae8440.os  "  . $b;   // 전체 레코드수를 파악하기 위함.					 				
		  }
			else						 
		  			 
        { // 필드별 검색하기
					  $sql ="select * from mirae8440.os where ((workplacename like '%$search%' ) or (firstordman like '%$search%' )  or (secondordman like '%$search%' )  or (chargedman like '%$search%' ) ";
					  $sql .="or (firstord like '%$search%' ) or (secondord like '%$search%' ) or (worker like '%$search%' ) or (memo like '%$search%' ) or (memo2 like '%$search%' ) ) " . $a;
					  
                      $sqlcon ="select * from mirae8440.os where ((workplacename like '%$search%' )  or (firstordman like '%$search%' )  or (secondordman like '%$search%' )  or (chargedman like '%$search%' ) ";
