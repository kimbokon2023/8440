...
<style>
    ...
    @media (max-width: 768px) {
        ...
        * {
        ...
        .d-flex.mt-3.mb-1.justify-content-center h3 {
...
<div class="container justify-content-center">  
 ...
 <div class="d-flex mt-3 mb-1 justify-content-center">  
  <h3> <?$page_title?> </h3>  
     ...
     <?php include 'safety_firstresult.htm'; ?>
...
<div class="card-body">
 ...
 <div class="d-flex mt-3 mb-3 justify-content-center">  
    <span class="fs-5" > <?=$page_title?> </span>
  </div>	 
...
$start_num=$total_row;  
 ...
 ?>
	<tr onclick="redirectToView('<?= htmlspecialchars($item_num, ENT_QUOTES, 'UTF-8') ?>', '<?= htmlspecialchars($tablename, ENT_QUOTES, 'UTF-8') ?>')">      
	  ...
	  <td data-label="자료명">  <?= $item_subject ?>   </td>
...
var safetypageNumber; // 현재 페이지 번호 저장을 위한 전역 변수
...
function renderMobileCards() {
    ...
    $(visibleRows).each(function() {
        ...
        if ($row.hasClass('odd') || $row.hasClass('even')) {
            ...
            var subject = $row.find('td:eq(1)').html() || '';
            var nick = $row.find('td:eq(2)').text().trim();
            var date = $row.find('td:eq(3)').text().trim();
            var hit = $row.find('td:eq(4)').text().trim();
            var onclickAttr = $row.attr('onclick');
            var itemNum = '';
            var tablename = '<?= htmlspecialchars($tablename, ENT_QUOTES, 'UTF-8') ?>';
            ...
            cardHtml += '<div class="mobile-card-item"><span class="mobile-card-label">자료명</span><span class="mobile-card-value">' + subject + '</span></div>';
            cardHtml += '<div class="mobile-card-item"><span class="mobile-card-label">등록인</span><span class="mobile-card-value">' + escapeHtml(nick) + '</span></div>';
            cardHtml += '<div class="mobile-card-item"><span class="mobile-card-label">등록일자</span><span class="mobile-card-value">' + escapeHtml(date) + '</span></div>';
            cardHtml += '<div class="mobile-card-item"><span class="mobile-card-label">조회수</span><span class="mobile-card-value">' + escapeHtml(hit) + '</span></div>';
            cardHtml += '</div>';
...
$(document).ready(function() {			
    ...
    // 페이지 번호 복원 (초기 로드 시)
    var savedPageNumber = getCookie('safetypageNumber');
    if (savedPageNumber) {
    ...
    dataTable.on('page.dt', function() {
        var safetypageNumber = dataTable.page.info().page + 1;
        setCookie('safetypageNumber', safetypageNumber, 10); // 쿠키에 페이지 번호 저장
        if (window.innerWidth <= 768) {
            setTimeout(renderMobileCards, 100);
        }
    });
    ...
    $('#myTable_length select').on('change', function() {
        ...
        // 변경 후 현재 페이지 번호 복원
        savedPageNumber = getCookie('safetypageNumber');
        if (savedPageNumber) {
            dataTable.page(parseInt(savedPageNumber) - 1).draw(false);
        }
    });
...
function redirectToView(num, tablename) {
    var page = safetypageNumber; // 현재 페이지 번호 (+1을 해서 1부터 시작하도록 조정)
    	
    var url = "view.php?num=" + num + "&tablename=" + tablename;          

	customPopup(url, '사고사망핵심위험요인', 1200, 900); 		    
}
...
$(document).ready(function(){
	...
	$("#writeBtn").click(function(){ 
		...
		customPopup(url, '사고사망핵심위험요인', 1300, 850); 	
	 });			 
...
// 서버에 작업 기록
$(document).ready(function(){
	saveLogData('사고사망핵심위험요인'); // 다른 페이지에 맞는 menuName을 전달
});
</script> 
</body>
</html>
