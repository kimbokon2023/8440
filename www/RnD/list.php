...
<style>
    ...
    @media (max-width: 768px) {
        ...
        * {
        ...
        /* 이미지 최적화 */
        .d-flex.mt-3.mb-1.justify-content-center img {
            width: 100% !important;
        ...
        .d-flex.mt-3.mb-1.justify-content-center h3 a {
            font-size: 1rem !important;
            word-wrap: break-word !important;
            overflow-wrap: break-word !important;
        }
...
<div class="card-body">
 ...
 <div class="d-flex mt-3 mb-1 justify-content-center">  
    <img src="../img/labbanner.jpg" style="width:100%;height:200px;">
  </div>	 
 <div class="d-flex mt-3 mb-1 justify-content-center">  
  <h3> <a href="https://www.rnd.or.kr/user/main.do" target="_blank">기업부설연구소 신고관리시스템 들어가기</a></h3>  
  </div>	 
...
var RnDpageNumber; // 현재 페이지 번호 저장을 위한 전역 변수
...
$(document).ready(function() {			
    ...
    // 페이지 번호 복원 (초기 로드 시)
    var savedPageNumber = getCookie('RnDpageNumber');
    if (savedPageNumber) {
    ...
    dataTable.on('page.dt', function() {
        var RnDpageNumber = dataTable.page.info().page + 1;
        setCookie('RnDpageNumber', RnDpageNumber, 10); // 쿠키에 페이지 번호 저장
        if (window.innerWidth <= 768) {
            setTimeout(renderMobileCards, 100);
        }
    });
    ...
    $('#myTable_length select').on('change', function() {
        ...
        // 변경 후 현재 페이지 번호 복원
        savedPageNumber = getCookie('RnDpageNumber');
        if (savedPageNumber) {
            dataTable.page(parseInt(savedPageNumber) - 1).draw(false);
        }
    });
...
function redirectToView(num, tablename) {
    var page = RnDpageNumber; // 현재 페이지 번호 (+1을 해서 1부터 시작하도록 조정)
    	
    var url = "view.php?num=" + num + "&tablename=" + tablename;          

	customPopup(url, '연구소', 1200, 900); 		    
}
...
// 서버에 작업 기록
$(document).ready(function(){
	saveLogData('연구소 게시판'); // 다른 페이지에 맞는 menuName을 전달
});
</script> 
</body>
</html>