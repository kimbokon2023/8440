  ...
  if(isset($_REQUEST["mode"]))
   ...
   if($mode=="search"){
         if(!$search) {
              ...
              $sql="select * from mirae8440." . $tablename . " where name like '%$search%' or subject like '%$search%'  or nick like '%$search%'   or searchtext like '%$search%'  or regist_day like '%$search%'  order by num desc   ";              
       } else {
              $sql="select * from mirae8440." . $tablename . " order  by num desc  ";
       }
...
function redirectToView(num, tablename) {
    var page = HRpageNumber; // 현재 페이지 번호 (+1을 해서 1부터 시작하도록 조정)
