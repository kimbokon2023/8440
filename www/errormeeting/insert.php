...
if ($filedelete == 'before') {
    ...
    try {
        ...
        $sql = "UPDATE {$DB}.emeeting SET {$delfile} WHERE num = ? LIMIT 1";
...
if ($mode == "modify" || $mode == "insert") {
    ...
    // 필드명 배열
    array_push($fieldarr, 'place', 'occur', 'errortype', 'emember', 'content', 
               'method', 'filename', 'serverfilename', 'approve');
...
if ($mode == "delete") {
    try {
        $sql = "SELECT serverfilename FROM {$DB}.emeeting WHERE num = ?";
        $stmh = $pdo->prepare($sql);
        $stmh->bindValue(1, $num, PDO::PARAM_INT);
        $stmh->execute();
        $row = $stmh->fetch(PDO::FETCH_ASSOC);
    ...
    try {
        ...
        $sql = "DELETE FROM {$DB}.emeeting WHERE num = ?";
        $stmh = $pdo->prepare($sql);
        $stmh->bindValue(1, $num, PDO::PARAM_INT);
        $stmh->execute();
