...
// 체크리스트 배열 저장 - section1~7
$sections = ['section1', 'section2', 'section3', 'section4', 'section5', 'section6', 'section7'];
$isSection = in_array($item, $sections);

if ($isSection && $term == '주간') {
...
if ($isSection && $term == '1개월') {
    array_push($question, "책상 위에 있는 먼지나 쓰레기를 청소했나요?");
    array_push($question, "책상 서랍이나 선반에 있는 쓰레기를 비웠나요?");
    array_push($question, "필요 없는 문구류나 소품들은 제거했나요?");
}

if ($isSection && $term == '2개월') {
    array_push($question, "책상 위에 있는 먼지나 쓰레기를 청소했나요?");
    array_push($question, "책상 서랍이나 선반에 있는 쓰레기를 비웠나요?");
    array_push($question, "필요 없는 문구류나 소품들은 제거했나요?");
}

if ($isSection && $term == '6개월') {
    array_push($question, "책상 위에 있는 먼지나 쓰레기를 청소했나요?");
    array_push($question, "책상 서랍이나 선반에 있는 쓰레기를 비웠나요?");
    array_push($question, "필요 없는 문구류나 소품들은 제거했나요?");
}
                                ...
                                <div class="card-body p-4">
                                    <div class="d-flex justify-content-center align-items-center mb-4">
                                        ...
                                        <p class="fw-normal mb-0" style="color: #a8729a;">
                                            <button type="button" class="btn btn-secondary btn-sm" onclick="write_memo('<?= $num ?>');">기록 저장</button>
                                        </p>
                                    </div>
                                    <!-- 대분류 끝 -->
...
<script>
    ...
    function checklist(num, whichone) {
        ...
        if (writer == user_name || writer2 == user_name || user_name === '김보곤') {
            // 로그인 이름과 같을때는 기록한다.
            // DB 수정
            $("#table").val('myarealist');
            $("#command").val('update');
