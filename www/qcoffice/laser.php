  ...
  for($k=0; $k<count($period); $k++)   // 주간/1개월/2개월/6개월  4개항목 반복문
	{
	  ...
	  for($j=1; $j<=(int)$arrtmp[$k]; $j++)
	  {
	   ...
	   if($k==count($period)-1)
		...
		$sql="select * from mirae8440.myarealist where " . $sqladd . $a; 					
	...
	for($j=0;$j<count($mcno_arr);$j++)
	 ...
	 // var_dump('배열점검');
	  ...
	  for($j=1; $j<=(int)$arrtmp[0]; $j++)
	  {
		...
		$sql="select * from mirae8440.myarealist where (item='" . $mcno . "'" . $sqladd . $a; 							
  ...
  for($k=0; $k<count($period); $k++)   // 주간/1개월/2개월/6개월  4개항목 반복문
	{
	  ...
	  for($j=1; $j<=(int)$arrtmp[$k]; $j++)
	  {
	   ...
	   if($k==count($period)-1)
		...
		$sql="select * from mirae8440.myarealist where " . $sqladd . $a; 					
		$sqlcon = "select * from mirae8440.myarealist where " . $sqladd . $b; 	
	...
	<div class="d-flex align-items-center mt-3 mb-1 justify-content-start">         
		<select class="form-control me-2" name="mcno" id="mcno" style="width:12%;" >		  
		   ...
		   ?>      
			...
			<span class="form-control me-2" style="width:25%;">
				장비점검 List &nbsp;&nbsp; (정) <?=$mcmain?>, (부) <?=$mcsub?> 
			</span>
...
var officemcpageNumber; // 현재 페이지 번호 저장을 위한 전역 변수
...
$(document).ready(function() {			
    ...
    // 페이지 번호 복원 (초기 로드 시)
    var savedPageNumber = getCookie('officemcpageNumber');
    if (savedPageNumber) {
    ...
    dataTable.on('page.dt', function() {
        var officemcpageNumber = dataTable.page.info().page + 1;
        setCookie('officemcpageNumber', officemcpageNumber, 10); // 쿠키에 페이지 번호 저장
    });
    ...
    $('#myTable_length select').on('change', function() {
        ...
        // 변경 후 현재 페이지 번호 복원
        savedPageNumber = getCookie('officemcpageNumber');
        if (savedPageNumber) {
            dataTable.page(parseInt(savedPageNumber) - 1).draw(false);
        }
    });
