...
var RiskAssessmentpageNumber; // 현재 페이지 번호 저장을 위한 전역 변수
...
$(document).ready(function() {
    ...
    // 페이지 번호 복원 (초기 로드 시)
    var savedPageNumber = getCookie('RiskAssessmentpageNumber');
    if (savedPageNumber) {
    ...
    dataTable.on('page.dt', function() {
        var RiskAssessmentpageNumber = dataTable.page.info().page + 1;
        setCookie('RiskAssessmentpageNumber', RiskAssessmentpageNumber, 10); // 쿠키에 페이지 번호 저장
        if (window.innerWidth <= 768) {
            setTimeout(renderMobileCards, 100);
        }
    });
    ...
    $('#myTable_length select').on('change', function() {
        ...
        // 변경 후 현재 페이지 번호 복원
        savedPageNumber = getCookie('RiskAssessmentpageNumber');
        if (savedPageNumber) {
            dataTable.page(parseInt(savedPageNumber) - 1).draw(false);
        }
    });
    ...
    $("#writeBtn").click(function() {
        var page = RiskAssessmentpageNumber;
