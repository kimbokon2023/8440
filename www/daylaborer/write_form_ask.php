<?php
require_once __DIR__ . '/../bootstrap.php';
require_once getDocumentRoot() . '/session.php';

// 세션 변수 초기화
$level = $_SESSION["level"] ?? 5;
$user_name = $_SESSION["name"] ?? '';
$user_id = $_SESSION["userid"] ?? '';
$DB = $_SESSION["DB"] ?? 'mirae8440';

// 권한 체크
if (!isset($_SESSION["level"]) || $level > 5) {
    sleep(1);
    
    // 동적 리다이렉트 (로컬/서버 환경)
    $isLocal = (strpos($_SERVER['HTTP_HOST'], '127.0.0.1') !== false || strpos($_SERVER['HTTP_HOST'], 'localhost') !== false);
    $baseUrl = $isLocal ? 'http://8440.local' : 'http://8440.co.kr';
    
    header("Location: {$baseUrl}/login/logout.php");
    exit;
}

// 요청 파라미터 초기화
$num = $_REQUEST["num"] ?? '';

// 데이터베이스 연결
require_once(includePath('lib/mydb.php'));
$pdo = db_connect();

$writtenarr = array();
$contentsarr = array();

// 같은 날짜 중복입력을 방지하기 위해서 입력날을 체크한다.
try {
    $sql = "SELECT * FROM {$DB}.daylaborer";
    $stmh = $pdo->query($sql);
    
    while ($row = $stmh->fetch(PDO::FETCH_ASSOC)) {
        array_push($writtenarr, $row["askdatefrom"]);
        array_push($contentsarr, $row["content"]);
    }
} catch (PDOException $ex) {
    error_log("일용직 중복 체크 조회 오류: " . $ex->getMessage());
}

// rowDBask.php에서 사용되는 변수 초기화
$name = '';
$askdatefrom = '';
$askdateto = '';
$content = '';
$item = '';
$labor_name = '';
$part = '';
$state = '';
$registdate = '';

// 수정 모드인 경우 데이터 조회
if ($num != '') {
    try {
        $sql = "SELECT * FROM {$DB}.daylaborer WHERE num = ?";
        $stmh = $pdo->prepare($sql);
        $stmh->bindValue(1, $num, PDO::PARAM_STR);
        $stmh->execute();
        $count = $stmh->rowCount();
        $row = $stmh->fetch(PDO::FETCH_ASSOC);
        
        if ($count > 0) {
            include 'rowDBask.php';
        }
    } catch (PDOException $ex) {
        error_log("일용직 데이터 조회 오류: " . $ex->getMessage());
    }
}

// 배열로 기본정보 불러옴
include "load_DB.php";

// 기본정보 배열 초기화 (load_DB.php에서 설정되지 않은 경우 대비)
$basic_name_arr = $basic_name_arr ?? array();
$basic_part_arr = $basic_part_arr ?? array();
$totalname_arr = $totalname_arr ?? array();
$totalused_arr = $totalused_arr ?? array();
$availableday_arr = $availableday_arr ?? array();

// 신규 데이터인 경우 초기값 설정
if ($num == '') {
    $registdate = date("Y-m-d");
    $askdatefrom = date("Y-m-d");
    $askdateto = date("Y-m-d");
    $usedday = abs(strtotime($askdateto) - strtotime($askdatefrom)) + 1;  // 날짜 빼기 계산
    $item = '';
    $state = '결재상신';
    $name = '';
    $part = '';
    
    // DB에서 part 찾아서 넣어주기
    for ($i = 0; $i < count($basic_name_arr); $i++) {
        if (trim($basic_name_arr[$i]) == trim($name)) {
            $part = $basic_part_arr[$i];
            break;
        }
    }
}

// 잔여일수 개인별 산출 루틴
$totalusedday = 0;
$totalremainday = 0;
$availableday = 0;

try {
    // 연차 잔여일수 산출
    for ($i = 0; $i < count($totalname_arr); $i++) {
        if ($name == $totalname_arr[$i]) {
            $availableday = $availableday_arr[$i] ?? 0;
        }
    }
    
    // 연차 사용일수 계산
    for ($i = 0; $i < count($totalname_arr); $i++) {
        if ($name == $totalname_arr[$i]) {
            $totalusedday = $totalused_arr[$i] ?? 0;
            $totalremainday = $availableday - $totalusedday;
        }
    }
} catch (PDOException $ex) {
    error_log("잔여일수 계산 오류: " . $ex->getMessage());
}

// 배열로 기본정보 불러옴
include "load_DB.php";

// 기본정보 배열 초기화 (load_DB.php에서 설정되지 않은 경우 대비)
$basic_name_arr = $basic_name_arr ?? array();
$basic_part_arr = $basic_part_arr ?? array();
$totalname_arr = $totalname_arr ?? array();
$totalused_arr = $totalused_arr ?? array();
$availableday_arr = $availableday_arr ?? array();

// 신규 데이터인 경우 초기값 설정
if ($num == '') {
    $registdate = date("Y-m-d");
    $askdatefrom = date("Y-m-d");
    $askdateto = date("Y-m-d");
    $usedday = 1;  // 기본 1일
    $item = '';
    $state = '요청';
    $name = '';
    $part = '';
}

// 잔여일수 개인별 산출 루틴
$totalusedday = 0;
$totalremainday = 0;
$availableday = 0;

// 연차 잔여일수 산출
for ($i = 0; $i < count($totalname_arr); $i++) {
    if ($name == $totalname_arr[$i]) {
        $availableday = $availableday_arr[$i] ?? 0;
    }
}

// 연차 사용일수 계산
for ($i = 0; $i < count($totalname_arr); $i++) {
    if ($name == $totalname_arr[$i]) {
        $totalusedday = $totalused_arr[$i] ?? 0;
        $totalremainday = $availableday - $totalusedday;
    }
}

$name_arr = array_unique($totalname_arr);
$today = date("Y-m-d");

// 관리자 권한 체크
$admin_names = array('이경묵', '김보곤', '조경임', '소민지');
$is_admin = in_array($user_name, $admin_names);

?>

<?php include getDocumentRoot() . '/load_header.php' ?>

<title>일용직 근태 신청</title>

</head>

<body>

<!-- Modal -->
<div class="modal fade" id="myModal" role="dialog">
    <div class="modal-dialog modal-lg modal-center">
        <!-- Modal content-->
        <div class="modal-content modal-lg">
            <div class="modal-header">
                <h4 class="modal-title">알림</h4>
            </div>
            <div class="modal-body">
                <div id="alertmsg" class="fs-1 mb-5 justify-content-center">
                    결재가 진행중입니다. <br>
                    <br>
                    수정사항이 있으면 결재권자에게 말씀해 주세요.
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" id="closeModalBtn" class="btn btn-default" data-dismiss="modal">닫기</button>
            </div>
        </div>
    </div>
</div>

<form id="board_form" name="board_form" class="form-signin" method="post">
    <input type="hidden" id="mode" name="mode">
    <input type="hidden" id="num" name="num" value="<?= htmlspecialchars($num) ?>">
    <input type="hidden" id="registdate" name="registdate" value="<?= htmlspecialchars($today) ?>">
    <input type="hidden" id="user_name" name="user_name" value="<?= htmlspecialchars($user_name) ?>">
    <input type="hidden" id="item" name="item" value="1">
    
    <div class="container" style="width:420px;">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title text-center" style="color:#113366;">일용직 근태</h5>
            </div>
            <div class="card-body text-center">
                <h6 class="form-signin-heading mt-1 mb-1">근무형태</h6>
                <div class="d-flex justify-content-center mt-1 mb-3">
                    <select name="content" id="content" class="form-select form-select-sm w-auto text-center mx-1">
                        <?php
                        $content_arr = array("일당");
                        
                        for ($i = 0; $i < count($content_arr); $i++) {
                            if ($content == $content_arr[$i]) {
                                print "<option selected value='" . $content_arr[$i] . "'>" . $content_arr[$i] . "</option>";
                            } else {
                                print "<option value='" . $content_arr[$i] . "'>" . $content_arr[$i] . "</option>";
                            }
                        }
                        ?>
                    </select>
                </div>
                
                <h6 class="form-signin-heading mt-3 mb-1">근무일자</h6>
                <div class="d-flex justify-content-center">
                    <input type="date" class="form-control text-center w-auto" id="askdatefrom" name="askdatefrom" 
                           required autofocus value="<?= htmlspecialchars($askdatefrom) ?>">
                </div>
                
                <h6 class="form-signin-heading mt-3 mb-2">성함</h6>
                <div class="d-flex justify-content-center">
                    <input type="text" class="form-control text-center me-1" style="width:30%;" id="labor_name" 
                           name="labor_name" required autofocus value="<?= htmlspecialchars($labor_name) ?>">
                    <button id="load_name" class="btn btn-dark btn-sm" type="button">
                        <i class="bi bi-search"></i>
                    </button>
                </div>
                
                <h6 class="form-signin-heading mt-3 mb-2">요청/요청확인/지급완료</h6>
                <div class="d-flex justify-content-center">
                    <select name="state" id="state" class="form-select form-select-sm w-auto text-center mx-1">
                        <?php
                        $state_arr = array('요청', '요청확인', '지급완료');
                        
                        if ($state == '') {
                            $state = "요청";
                        }
                        
                        for ($i = 0; $i < count($state_arr); $i++) {
                            if ($state === $state_arr[$i]) {
                                print "<option selected value='" . $state_arr[$i] . "'>" . $state_arr[$i] . "</option>";
                            } else {
                                print "<option value='" . $state_arr[$i] . "'>" . $state_arr[$i] . "</option>";
                            }
                        }
                        ?>
                    </select>
                </div>
                
                <h6 class="form-signin-heading mt-3 mb-2">비고(전번/추가근무)</h6>
                <div class="d-flex justify-content-center">
                    <input type="text" class="form-control text-center" style="width:50%;" id="part" name="part" 
                           autocomplete="off" autofocus value="<?= htmlspecialchars($part) ?>">
                </div>
                
                <div class="d-flex justify-content-center mt-4 mb-2">
                    <?php if ($is_admin) { ?>
                        <button id="saveBtn" class="btn btn-dark btn-sm me-2" type="button">
                            <i class="bi bi-floppy-fill"></i> 저장
                        </button>
                    <?php } ?>
                    
                    <?php if ((int)$num > 0 && $is_admin) { ?>
                        <button id="delBtn" class="btn btn-danger btn-sm" type="button">
                            <i class="bi bi-trash"></i> 삭제
                        </button>
                    <?php } ?>
                    
                    <button id="closeBtn" type="button" onclick="self.close();" class="btn btn-dark btn-sm ms-5">
                        &times; 닫기
                    </button>
                </div>
            </div>
        </div>
    </div>
</form>

<script>
$(document).ready(function() {
    var state = $('#state').val();
    
    // 모달 닫기
    $("#closeModalBtn").click(function() {
        $('#myModal').modal('hide');
    });
    
    // 근무형태 변경 시
    $("#content").change(function() {
        var val = $('select[name="content"]').val();
        console.log(val);
    });
    
    // 이름 검색 팝업
    $("#load_name").click(function() {
        var href = './load_name.php?tablename=daylaborer';
        popupCenter(href, '일용직 이름 검색', 550, 500);
    });
				
    // 저장 버튼
    $("#saveBtn").click(function() {
        // 일자정보를 배열에 넣고 첫번째줄의 요소를 바꿔준다.
        var writtenarr = <?php echo json_encode($writtenarr, JSON_UNESCAPED_UNICODE); ?>;
        var contentsarr = <?php echo json_encode($contentsarr, JSON_UNESCAPED_UNICODE); ?>;
        
        var num = $("#num").val();
        var part = $("#part").val();
        var state = $("#state").val();
        var user_name = $("#user_name").val();
        var askdatefrom = $("#askdatefrom").val();
        var content = $("#content").val();
        
        // 관리자만 저장 가능
        if (user_name == '김보곤' || user_name == '이경묵' || user_name == '조경임' || user_name == '소민지') {
            if (Number(num) > 0) {
                $("#mode").val('modify');
            } else {
                $("#mode").val('insert');
            }
            
            $.ajax({
                url: "insert_ask.php",
                type: "post",
                data: $("#board_form").serialize(),
                dataType: "json",
                success: function(data) {
                    console.log(data);
                    opener.location.reload();
                    window.close();
                },
                error: function(jqxhr, status, error) {
                    console.log(jqxhr, status, error);
                }
            });
        } else {
            $('#myModal').modal('show');
        }
    });
    
    // 삭제 버튼
    $("#delBtn").click(function() {
        var num = $("#num").val();
        var state = $("#state").val();
        var user_name = $("#user_name").val();
        
        // 관리자만 삭제 가능
        if (state == '요청' || user_name == '김보곤' || user_name == '이경묵' || user_name == '조경임' || user_name == '소민지') {
            $("#mode").val('delete');
            
            // DATA 삭제 확인
            Swal.fire({
                title: '해당 DATA 삭제',
                text: "DATA 삭제는 신중하셔야 합니다.\n정말 삭제 하시겠습니까?",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                confirmButtonText: '삭제',
                cancelButtonText: '취소'
            }).then(function(result) {
                if (result.isConfirmed) {
                    $.ajax({
                        url: "insert_ask.php",
                        type: "post",
                        data: $("#board_form").serialize(),
                        dataType: "json",
                        success: function(data) {
                            console.log(data);
                            
                            if (data.success) {
                                Toastify({
                                    text: data.message || "삭제 완료!",
                                    duration: 3000,
                                    close: true,
                                    gravity: "top",
                                    position: "center",
                                    backgroundColor: "#4fbe87"
                                }).showToast();
                                
                                setTimeout(function() {
                                    opener.location.reload();
                                    window.close();
                                }, 1000);
                            } else {
                                Toastify({
                                    text: data.message || "삭제 중 오류가 발생했습니다.",
                                    duration: 3000,
                                    close: true,
                                    gravity: "top",
                                    position: "center",
                                    backgroundColor: "#ff6b6b"
                                }).showToast();
                            }
                        },
                        error: function(jqxhr, status, error) {
                            console.log(jqxhr, status, error);
                            Toastify({
                                text: "서버 오류가 발생했습니다.",
                                duration: 3000,
                                close: true,
                                gravity: "top",
                                position: "center",
                                backgroundColor: "#ff6b6b"
                            }).showToast();
                        }
                    });
                }
            });
        }
    });
});

// 두 날짜 사이 일자 구하기 (ES5 호환)
function getDateDiff(d1, d2) {
    var date1 = new Date(d1);
    var date2 = new Date(d2);
    
    var diffDate = date1.getTime() - date2.getTime();
    
    return Math.abs(diffDate / (1000 * 60 * 60 * 24)); // 밀리세컨 * 초 * 분 * 시 = 일
}
</script>
</body>
</html>
