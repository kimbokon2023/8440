...
require_once(includePath('lib/mydb.php'));
 ...
 if ($mode=="modify"){      
     try{
        $sql = "select * from mirae8440.daylaborer where num=?";  // get target record
     ...
     try{
        ...
        $sql = "update mirae8440.daylaborer set id=?, name=?, registdate=?, item=?, askdatefrom=?,  askdateto=?,  usedday=?,  content=?,  state=?, part=?, labor_name=? ";
 ...
 if ($mode=="insert"){	 	 
   try{
     ...
     $sql = "insert into mirae8440.daylaborer(id , name , registdate , item , askdatefrom ,  askdateto ,  usedday ,  content ,  state, part , labor_name) ";     
     $sql .= " values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ";
 ...
 if ($mode=="delete"){	 	 
   try{
     ...
     $sql = "delete from  mirae8440.daylaborer where num = ?";  
     $stmh = $pdo->prepare($sql);
     $stmh->bindValue(1,$num,PDO::PARAM_STR);      
     $stmh->execute();   
     $pdo->commit();	 
